...
// ----------------------------------------------------------------------------
//  ImGui Setup: Created separate renderpass for ImGui, instead of clearing
//  framebuffer values from previous pass, draw ImGui overlay on top of it
//  (createRenderPass). When rendering in renderFrame function, setup
//  commandbuffer to record ImGui specific commands and submit it with other
//  drawing commands.
//
...
{
    IMGUI_CHECKVERSION();
    m_ui = std::make_unique<ImGuiOverlay>(m_Device.get(), m_Renderpass);
}
...
void Context::updateOverlay(float dt)
{
    ...
    // ImGui::ColorEdit3("Color", &ubo.color.x);
...
{
    DescriptorSetGenerator gen(m_Device->getLogicalDevice());
    ...
    m_DescriptorPool = gen.generatePool(100);
    m_DescSetLayout = gen.generateLayout();
    ...
    for(std::size_t i = 0; i < m_DescriptorSet.size(); ++i)
    {
        ...
        gen.bind(m_DescriptorSet[i], 0, {bufferInfo});
        gen.bind(m_DescriptorSet[i], 1, imageInfos);
        gen.bind(m_DescriptorSet[i], 2, materialBufferInfos);
    }

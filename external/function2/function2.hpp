...
struct is_noexcept_correct<true, T, identity<Args...>> :
    std::integral_constant<
            bool,
            noexcept(invoke(std::declval<T>(), std::declval<Args>()...))>
{
};
...
template<bool IsCopyable, typename T, typename Allocator>
auto make_box(
        std::integral_constant<bool, IsCopyable>,
        T&& value,
        Allocator&& allocator)
{
...
using transfer_const_t = std::conditional_t<
        std::is_const<std::remove_pointer_t<From>>::value,
        std::add_const_t<To>,
        To>;
template<typename From, typename To>
using transfer_volatile_t = std::conditional_t<
        std::is_volatile<std::remove_pointer_t<From>>::value,
        std::add_volatile_t<To>,
        To>;

/// The retriever when the object is allocated inplace
template<typename T, typename Accessor>
constexpr auto retrieve(
        std::true_type /*is_inplace*/, Accessor from, std::size_t from_capacity)
{
    using type =
...
#define FU2_DEFINE_FUNCTION_TRAIT(CONST, VOLATILE, NOEXCEPT, OVL_REF, REF)     \
    ...
    {                                                                          \
        using pointer_type = Ret (*)(                                          \
                data_accessor CONST VOLATILE*, std::size_t capacity, Args...); \
        template<typename T, bool IsInplace>                                   \
        struct internal_invoker                                                \
        {                                                                      \
            static Ret invoke(                                                 \
                    data_accessor CONST VOLATILE* data,                        \
                    std::size_t capacity,                                      \
                    Args... args) NOEXCEPT                                     \
            {                                                                  \
                auto obj = retrieve<T>(                                        \
                        std::integral_constant<bool, IsInplace>{},             \
                        data,                                                  \
                        capacity);                                             \
                auto box = static_cast<T CONST VOLATILE*>(obj);                \
                return invocation::invoke(                                     \
                        static_cast<std::decay_t<decltype(box->value_)> CONST  \
                                            VOLATILE REF>(box->value_),        \
                        std::forward<Args>(args)...);                          \
            }                                                                  \
        };                                                                     \
                                                                               \
        ...
        {                                                                      \
            ...
            {                                                                  \
                throw_or_abort##NOEXCEPT(                                      \
                        std::integral_constant<bool, Throws>{});               \
            }                                                                  \
        };                                                                     \
    };
...
struct invoke_table<First>
{
    ...
    {
        return &function_trait<
                First>::template internal_invoker<T, IsInplace>::invoke;
    }
    /// Returns the thunk of an single overloaded callable
    template<typename T>
    ...
    {
        return &function_trait<First>::template empty_invoker<
                IsThrowing>::invoke;
    }
};
                ...
                function_pointer_of<Args>...>
    {
        constexpr empty_vtable() noexcept :
            std::tuple<
                    ...
                    function_pointer_of<Second>,
                    function_pointer_of<Args>...>(std::make_tuple(
                    &function_trait<First>::template empty_invoker<
                            IsThrowing>::invoke,
...
#define FU2_DEFINE_FUNCTION_TRAIT(CONST, VOLATILE, NOEXCEPT, OVL_REF, REF)     \
    ...
    protected:                                                                 \
        ...
        operator_impl& operator=(operator_impl&&) = default;                   \
                                                                               \
        using operator_impl<Index + 1, Function, Next, Signatures...>::        \
                operator();                                                    \
                                                                               \
        ...
        {                                                                      \
            ...
            using erasure_t = std::decay_t<decltype(parent->erasure_)>;        \
                                                                               \
            /* `std::decay_t<decltype(parent->erasure_)>` is a workaround for  \
             * a   */                                                          \
            /* compiler regression of MSVC 16.3.1, see #29 for details. */                                                                            \
            return std::decay_t<decltype(parent->erasure_)>::template invoke<  \
                    Index>(                                                    \
                    static_cast<erasure_t CONST VOLATILE REF>(                 \
                            parent->erasure_),                                 \
                    std::forward<Args>(args)...);                              \
        }                                                                      \
    };                                                                         \
    ...
    protected:                                                                 \
        ...
        {                                                                      \
            auto parent =                                                      \
                    static_cast<function<Config, Property> CONST VOLATILE*>(   \
                            this);                                             \
            using erasure_t = std::decay_t<decltype(parent->erasure_)>;        \
                                                                               \
            /* `std::decay_t<decltype(parent->erasure_)>` is a workaround for  \
...
class vtable<property<IsThrowing, HasStrongExceptGuarantee, FormalArgs...>>
{
    ...
    {
        static_assert(
                is_box<T>::value, "The trait must be specialized with a box!");
                ...
                std::size_t to_capacity) noexcept(HasStrongExceptGuarantee)
        {
            ...
            else
            {
                ...
                to->ptr_ = storage =
                        box_factory<std::decay_t<Box>>::box_allocate(
                                std::addressof(box));
                to_table->template set_allocated<T>();
            }
...
public:
    ...
    /// Destroys the object at the given position
    void destroy(data_accessor* from, std::size_t from_capacity) noexcept(
            HasStrongExceptGuarantee)
    {
    ...
    void set_inplace() noexcept
    {
        ...
        vtable_ =
                invoke_table_t::template get_invocation_table_of<type, true>();
    ...
    void set_allocated() noexcept
    {
        using type = std::decay_t<T>;
        vtable_ =
                invoke_table_t::template get_invocation_table_of<type, false>();
        cmd_ = &trait<type>::template process_cmd<false>;
    }

    void set_empty() noexcept
    {
        vtable_ = invoke_table_t::template get_empty_invocation_table<
                IsThrowing>();
        cmd_ = &empty_cmd;
    }
};
...
struct internal_capacity<
        Capacity,
        std::enable_if_t<(Capacity::capacity < sizeof(void*))>>
{
    typedef struct
    {
        /// Tag to access the structure in a type-safe way
...
public:
    ...
    constexpr erasure& operator=(
            erasure<true, OtherConfig, Property>
                    right) noexcept(Property::is_strong_exception_guaranteed)
    {
        vtable_.weak_destroy(this->opaque_ptr(), capacity());
        right.vtable_.move(
                vtable_,
    ...
    static constexpr decltype(auto) invoke(Erasure&& erasure, Args&&... args)
    {
        ...
        return erasure.vtable_.template invoke<Index>(
                std::forward<Erasure>(erasure).opaque_ptr(),
                capacity,
                std::forward<Args>(args)...);
    }
};
...
template<
        /*bool IsOwning = false, */ typename Config,
        bool IsThrowing,
...
public:
    ...
    constexpr erasure() noexcept :
        invoke_table_(invoke_table_t::template get_empty_invocation_table<
                      IsThrowing>()),
        view_(nullptr)
    {
    }
    ...
    {
        auto thunk =
                invoke_table_t::template fetch<Index>(erasure.invoke_table_);
...
{
    static_assert(
            !Config::is_owning || !Config::is_copyable
...
template<typename LeftConfig, typename RightConfig>
using is_owning_correct = std::integral_constant<
        bool,
            ...
            Args...>
{
    ...
    template<typename T>
    using enable_if_can_accept_all_t = std::enable_if_t<
            accepts_all<std::decay_t<T>, identity<Args...>>::value>;
    ...
    using assert_no_strong_except_guarantee_t =
            typename assert_no_strong_except_guarantee<
                    HasStrongExceptGuarantee,
                    std::decay_t<T>>::type;
...
public:
    ...
    friend void swap(function& left, function& right) noexcept(
            HasStrongExceptGuarantee)
    {
        left.swap(right);
    }
    ...
    using type_erasure::invocation_table::
            operator_impl<0U, function<Config, property_t>, Args...>::operator();
};

...
typedef int ImGuiDataAuthority; // -> enum ImGuiDataAuthority_      // Enum: for
...
typedef int ImGuiSeparatorFlags; // -> enum ImGuiSeparatorFlags_     // Flags:
                                 // for SeparatorEx()
typedef int ImGuiTextFlags;    // -> enum ImGuiTextFlags_          // Flags: for
                               // TextEx()
...
// Helpers: Hashing
IMGUI_API ImGuiID
        ImHashData(const void* data, size_t data_size, ImU32 seed = 0);
IMGUI_API ImGuiID
        ImHashStr(const char* data, size_t data_size = 0, ImU32 seed = 0);
...
IMGUI_API int ImFormatStringV(
        char* buf, size_t buf_size, const char* fmt, va_list args)
        IM_FMTLIST(3);
...
IMGUI_API int ImTextCountUtf8BytesFromChar(
        const char* in_text,
        const char* in_text_end); // return number of bytes to express one char
                                  // in UTF-8
IMGUI_API int ImTextCountUtf8BytesFromStr(
        const ImWchar* in_text,
        const ImWchar* in_text_end); // return number of bytes to express string
                                     // in UTF-8
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    int CurLenW, CurLenA; // we need to maintain our buffer length in both UTF-8
                          // and wchar format. UTF-8 length is valid even if
                          // TextA is not.
...
struct ImGuiNextWindowData
{
    ...
    ImVec2 MenuBarOffsetMinVal; // (Always on) This is not exposed publicly, so
                                // we don't clear it and it doesn't have a
                                // corresponding flag (could we? for
                                // consistency?)
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    ImGuiDockNodeFlags_DockSpace =
            1 << 10, // Local, Saved  // A dockspace is a node that occupy space
                     // within an existing user window. Otherwise the node is
                     // floating and create its own window.
    ImGuiDockNodeFlags_CentralNode =
            1 << 11, // Local, Saved  // The central node has 2 main properties:
                     // stay visible when empty, only use "remaining" spaces
                     // from its neighbor.
    ImGuiDockNodeFlags_NoTabBar =
            1 << 12, // Local, Saved  // Tab bar is completely unavailable. No
                     // triangle in the corner to enable it back.
    ImGuiDockNodeFlags_HiddenTabBar =
            1 << 13, // Local, Saved  // Tab bar is hidden, with a triangle in
                     // the corner to show it again (NB: actual tab-bar instance
                     // may be destroyed as this is only used for single-window
                     // tab bar)
    ImGuiDockNodeFlags_NoWindowMenuButton =
            1 << 14, // Local, Saved  // Disable window/docking menu (that one
                     // that appears instead of the collapse button)
    ImGuiDockNodeFlags_NoCloseButton = 1 << 15, // Local, Saved  //
    ImGuiDockNodeFlags_NoDocking =
            1 << 16, // Local, Saved  // Disable any form of docking in this
                     // dockspace or individual node. (On a whole dockspace,
                     // this pretty much defeat the purpose of using a dockspace
                     // at all). Note: when turned on, existing docked nodes
                     // will be preserved.
    ImGuiDockNodeFlags_NoDockingSplitMe =
            1 << 17, // [EXPERIMENTAL] Prevent another window/node from
                     // splitting this node.
    ImGuiDockNodeFlags_NoDockingSplitOther =
            1 << 18, // [EXPERIMENTAL] Prevent this node from splitting another
                     // window/node.
    ImGuiDockNodeFlags_NoDockingOverMe =
            1 << 19, // [EXPERIMENTAL] Prevent another window/node to be docked
                     // over this node.
    ImGuiDockNodeFlags_NoDockingOverOther =
            1 << 20, // [EXPERIMENTAL] Prevent this node to be docked over
                     // another window/node.
    ...
    ImGuiDockNodeFlags_LocalFlagsTransferMask_ =
            ImGuiDockNodeFlags_LocalFlagsMask_
            & ~ImGuiDockNodeFlags_DockSpace, // When splitting those flags are
                                             // moved to the inheriting child,
                                             // never duplicated
...
struct IMGUI_API ImGuiDockNode
{
    ImGuiID ID;
    ImGuiDockNodeFlags
            SharedFlags; // Flags shared by all nodes of a same dockspace
    ...
    ImGuiID LastFocusedNodeId; // [Root node only] Which of our child docking
                               // node (any ancestor in the hierarchy) was last
                               // focused.
    ...
    bool MarkedForPosSizeWrite : 1; // Update by DockNodeTreeUpdatePosSize()
                                    // write-filtering
    ...
    bool IsFloatingNode() const
    {
        return ParentNode == NULL
               && (LocalFlags & ImGuiDockNodeFlags_DockSpace) == 0;
    }
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    int LastFrameActive; // Last frame number this viewport was activated by a
                         // window
    int LastFrontMostStampCount; // Last stamp number from when a window hosted
                                 // by this viewport was made front-most (by
                                 // comparing this value between two viewport we
                                 // have an implicit viewport z-order
...
struct ImGuiWindowSettings
{
    ...
    ImGuiID DockId;  // ID of last known DockNode (even if the DockNode is
                     // invisible because it has only 1 active window), or 0 if
                     // none.
    ...
    short DockOrder; // Order of the last time the window was visible within its
                     ...
                     // reappearing on the same frame. Same value between
                     // windows that were active and windows that were none are
                     // possible.
    bool Collapsed;
...
struct ImGuiSettingsHandler
{
    ...
    void (*ReadInitFn)(
            ImGuiContext* ctx,
            ImGuiSettingsHandler* handler); // Read: Called before reading (in
                                            // registration order)
    void* (*ReadOpenFn)(
            ImGuiContext* ctx,
    ...
    void (*ReadLineFn)(
            ...
            ImGuiSettingsHandler* handler,
            void* entry,
            const char* line); // Read: Called for every line of text within an
                               // ini entry
    void (*ApplyAllFn)(
            ImGuiContext* ctx,
...
struct ImGuiContext
{
    ...
    ImGuiStyle Style;
    ImGuiConfigFlags ConfigFlagsCurrFrame; // = g.IO.ConfigFlags at the time of
                                           // NewFrame()
    ...
    ImVector<ImGuiWindow*> WindowsFocusOrder; // Windows, sorted in focus order,
                                              // back to front. (FIXME: We could
                                              // only store root windows here!)
    ImVector<ImGuiWindow*>
    ...
    ImGuiWindow*
            MovingWindow; // Track the window we clicked on (in order to
                          // preserve focus). The actual window that is moved is
                          // generally MovingWindow->RootWindowDockTree.
    ...
    ImGuiID ActiveIdIsAlive; // Active widget has been seen this frame (we can't
                             // use a bool as the ActiveId may change within the
                             // frame)
    float ActiveIdTimer;
    ...
    ImU64 ActiveIdUsingKeyInputMask; // Active widget will want to read those key
                                     ...
                                     // we'll need to either to order the enum
                                     // to make useful keys come first, either
    ...
    ImGuiViewportP* CurrentViewport; // We track changes of viewport (happening
                                     // in Begin) so we can call
    ...
    int ViewportFrontMostStampCount; // Every time the front-most window
                                     // changes, we stamp its viewport with an
    ...
    ImGuiID NavInitResultId; // Init request result (first item of the window,
                             // or one for which SetItemDefaultFocus() was
                             // called)
    ...
    int LogDepthToExpandDefault; // Default/stored value for LogDepthMaxExpand
                                 // if not specified in the LogXXX function call.
...
// be called ImGuiDrawContext, hence the DC variable name in ImGuiWindow. (That's
// theory, in practice the delimitation between ImGuiWindow and
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImGuiNavLayer NavLayerCurrent; // Current layer, 0..31 (we currently only
                                   // use 0..1)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiViewportP* Viewport; // Always set in Begin(), only inactive windows
                              // may have a NULL value here
    ImGuiID ViewportId; // We backup the viewport id (since the viewport may
                        // disappear or never be created if the window is
                        // inactive)
    ...
    ImGuiCond SetWindowCollapsedAllowFlags : 8; // store acceptable condition
                                                // flags for
    ...
    ImDrawList* DrawList; // == &DrawListInst (for backward compatibility reason
                          // with code using imgui_internal.h we keep this a
                          // pointer)
    ...
    ImGuiWindow* RootWindow; // Point to ourself or first ancestor that is not a
                             // child window. Doesn't cross through dock nodes.
                             // We use this so IsWindowFocused() can behave
                             // consistently regardless of docking state.
    ImGuiWindow*
            RootWindowDockTree; // Point to ourself or first ancestor that is
                                // not a child window. Cross through dock nodes.
    ...
    ImGuiWindow*
            NavLastChildNavWindow; // When going to the menu bar, we remember
                                   // the child window we came from. (This could
    ...
    bool DockIsActive : 1; // When docking artifacts are actually visible. When
                           ...
                           // ~~ (DockNode != NULL) && (DockNode->Windows.Size >
                           // 1).
    ...
    ImGuiID DockId; // Backup of last valid DockNode->ID, so single window
                    // remember their dock node id even when they are not bound
                    // any more
...
enum ImGuiTabBarFlagsPrivate_
{
    ImGuiTabBarFlags_DockNode = 1
                                << 20, // Part of a dock node [we don't use this
                                       // in the master branch but it facilitate
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_Unsorted =
            1 << 22, // [Docking] Trailing tabs with the _Unsorted flag will be
                     // sorted based on the DockOrder of their Window.
    ImGuiTabItemFlags_Preview = 1
                                << 23 // [Docking] Display tab shape for docking
                                      // preview (height is adjusted slightly to
...
struct ImGuiTableColumn
{
    ImGuiTableColumnFlags
            Flags; // Flags after some patching (not directly same as provided
    ...
    ImGuiTableColumnIdx SortOrder; // Index of this column within sort specs, -1
                                   // if not sorting on this column, 0 for
...
struct ImGuiTable
{
    ...
    ImGuiTableFlags SettingsLoadedFlags; // Which data were loaded from the .ini
                                         // file (e.g. when order is not altered
                                         // we won't save order)
    ...
    ImGuiTableColumnIdx LastResizedColumn; // Index of column being resized from
                                           // previous frame.
    ...
    ImGuiTableDrawChannelIdx
            Bg2DrawChannelCurrent; // For Selectable() and other widgets drawing
                                   // across columns after the freezing line.
...
IMGUI_API void SetCurrentViewport(
...
IMGUI_API ImVec2
        ScrollToBringRectIntoView(ImGuiWindow* window, const ImRect& item_rect);
...
IMGUI_API void DockContextQueueDock(
        ...
        float split_ratio,
        bool split_outer);
IMGUI_API void DockContextQueueUndockWindow(
        ImGuiContext* ctx, ImGuiWindow* window);
IMGUI_API void DockContextQueueUndockNode(
...
IMGUI_API ImGuiID DockBuilderSplitNode(
        ...
        ImGuiID* out_id_at_opposite_dir); // Create 2 child nodes in this parent
                                          // node.
...
IMGUI_API int TableGetHoveredColumn(); // May use (TableGetColumnFlags() &
                                       ...
                                       // instead. Return hovered column. return
                                       // -1 when table is not hovered. return
                                       // columns_count if the unused space at
                                       // the right of visible columns is
                                       // hovered.
...
IMGUI_API const char* FindRenderedTextEnd(
        const char* text,
        const char* text_end = NULL); // Find the optional ## from which we stop
                                      // displaying text.
...
IMGUI_API bool TreeNodeBehaviorIsOpen(
        ImGuiID id,
        ImGuiTreeNodeFlags flags =
                0); // Consume previous SetNextItemOpen() data, if any. May

...
// [SECTION] Platform interface for multi-viewport support (ImGuiPlatformIO,
...
typedef void (*ImGuiSizeCallback)(
        ImGuiSizeCallbackData* data); // Callback function for
...
typedef unsigned short
        ImWchar16; // A single decoded U16 character/code point. We encode them
                   // as multi bytes UTF-8 when used in strings.
typedef unsigned int
        ImWchar32; // A single decoded U32 character/code point. We encode them
                   // as multi bytes UTF-8 when used in strings.
...
typedef unsigned __int64 ImU64; // 64-bit unsigned integer (pre and post C++11
                                // with Visual Studio)
...
IMGUI_API ImGuiStyle& GetStyle(); // access the Style structure (colors, sizes).
                                  // Always use PushStyleCol(), PushStyleVar()
...
IMGUI_API void EndFrame(); // ends the Dear ImGui frame. automatically called by
                           // Render(). If you don't need to render data
                           // (skipping rendering) you may call EndFrame()
                           // without Render()... but you'll have wasted CPU
                           // already! If you don't need to render, better to not
...
IMGUI_API const char* GetVersion(); // get the compiled version string e.g.
                                    // "1.80 WIP" (essentially the value for
                                    // IMGUI_VERSION from the compiled version
                                    // of imgui.cpp)
...
IMGUI_API bool IsWindowHovered(
        ImGuiHoveredFlags flags =
                0); // is current window hovered (and typically: not blocked by
                    // a popup/modal)? see flags for options. NB: If you are
                    // trying to check whether your mouse should be dispatched
                    // to imgui or to your app, you should use the
                    // 'io.WantCaptureMouse' boolean for that! Please read the
                    // FAQ!
IMGUI_API ImDrawList* GetWindowDrawList(); // get draw list associated to the
                                           // current window, to append your own
                                           // drawing primitives
...
IMGUI_API float GetWindowHeight(); // get current window height (shortcut for
                                   // GetWindowSize().y)
IMGUI_API ImGuiViewport* GetWindowViewport(); // get viewport currently
...
IMGUI_API void SetNextWindowSizeConstraints(
        ...
        void* custom_callback_data =
                NULL); // set next window size limits. use -1,-1 on either X/Y
                       // axis to preserve the current size. Sizes will be
                       // rounded down. Use callback to apply non-trivial
...
IMGUI_API void SetScrollFromPosX(
        ...
        float center_x_ratio =
                0.5f); // adjust scrolling amount to make given position
                       // visible. Generally GetCursorStartPos() + offset to
...
IMGUI_API float CalcItemWidth(); // width of item given pushed settings and
                                 // current cursor position. NOT necessarily the
                                 // width of last item unlike most 'Item'
                                 // functions.
...
IMGUI_API ImU32 GetColorU32(
        ImU32 col); // retrieve given color with style alpha applied, packed as
...
IMGUI_API void Dummy(
        const ImVec2& size); // add a dummy item of given size. unlike
                             // InvisibleButton(), Dummy() won't take the mouse
                             // click or be navigable into.
IMGUI_API void Indent(
        float indent_w =
                0.0f); // move content position toward the right, by indent_w,
                       // or style.IndentSpacing if indent_w <= 0
IMGUI_API void Unindent(
        float indent_w =
                0.0f); // move content position back to the left, by indent_w,
                       // or style.IndentSpacing if indent_w <= 0
...
IMGUI_API float GetCursorPosY(); //    other functions such as GetCursorScreenPos
                                 //    or everything in ImDrawList::
IMGUI_API void SetCursorPos(
        const ImVec2& local_pos); //    are using the main, absolute coordinate
                                  //    system.
...
IMGUI_API ImVec2
        GetCursorScreenPos(); // cursor position in absolute coordinates (useful
                              // to work with ImDrawList API). generally
                              // top-left == GetMainViewport()->Pos == (0,0) in
                              // single viewport mode, and bottom-right ==
...
IMGUI_API void AlignTextToFramePadding(); // vertically align upcoming text
                                          ...
                                          // it will align properly to regularly
                                          // framed items (call if you have text
...
IMGUI_API void PushID(
        ...
        const char* str_id_end); // push string into the ID stack (will hash
                                 // string).
...
IMGUI_API ImGuiID
        GetID(const char* str_id); // calculate unique ID (hash of whole ID
...
IMGUI_API void SetColorEditOptions(
        ImGuiColorEditFlags
                flags); // initialize current options (generally on application
                        // startup) if you want to select a default format,
                        // picker type, etc. User will be able to change many
                        // settings, unless you pass the _NoOptions flag to your
                        // calls.
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...)
        IM_FMTARGS(2); // helper variation to easily decorelate the id from the
                       ...
                       // use ID. to align arbitrary text at the same level as a
...
IMGUI_API bool CollapsingHeader(
        ...
        bool* p_visible,
        ImGuiTreeNodeFlags flags =
                0); // when 'p_visible != NULL': if '*p_visible==true' display
...
IMGUI_API bool Selectable(
        ...
        ImGuiSelectableFlags flags = 0,
        const ImVec2& size =
                ImVec2(0, 0)); // "bool* p_selected" point to the selection
...
IMGUI_API bool BeginPopupModal(
        ...
        ImGuiWindowFlags flags = 0); // return true if the modal is open, and
                                     // you can start outputting to it.
IMGUI_API void EndPopup(); // only call EndPopup() if BeginPopupXXX() returns
                           // true!
...
IMGUI_API void OpenPopupOnItemClick(
        ...
        ImGuiPopupFlags popup_flags =
                1); // helper to open popup when clicked on last item. Default
                    // to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually
                    // triggers on the mouse _released_ event to be consistent
                    // with popup behaviors)
IMGUI_API void CloseCurrentPopup(); // manually close the popup we have begin-ed
                                    // into.
...
IMGUI_API bool BeginPopupContextItem(
        const char* str_id = NULL,
        ImGuiPopupFlags popup_flags =
                1); // open+begin popup when clicked on last item. Use
...
IMGUI_API bool TableNextColumn(); // append into the next column (or first column
...
IMGUI_API void TableHeadersRow(); // submit all headers cells based on data
                                  // provided to TableSetupColumn() + submit
                                  // context menu
...
IMGUI_API ImGuiTableSortSpecs* TableGetSortSpecs(); // get latest sort specs for
                                                    // the table (NULL if not
                                                    // sorting).
// Tables: Miscellaneous functions
// - Functions args 'int column_n' treat the default value of -1 as the same as
// passing the current column index.
IMGUI_API int TableGetColumnCount(); // return number of columns (value passed
                                     // to BeginTable)
...
IMGUI_API ImGuiTableColumnFlags TableGetColumnFlags(
        int column_n = -1); // return column flags so you can query their
                            // Enabled/Visible/Sorted/Hovered status flags. Pass
                            // -1 to use current column.
IMGUI_API void TableSetColumnEnabled(
        int column_n,
        bool v); // change enabled/disabled state of a column, set to false to
                 // hide the column. Note that end-user can use the context menu
                 // to change this themselves (right-click in headers, or
...
IMGUI_API float GetColumnWidth(
        int column_index = -1); // get column width (in pixels). pass -1 to use
                                // current column
IMGUI_API void SetColumnWidth(
        int column_index, float width); // set column width (in pixels). pass -1
...
// (all windows). About DockSpace:
// - Use DockSpace() to create an explicit dock node _within_ an existing
// window. See Docking demo for details.
// - DockSpace() needs to be submitted _before_ any window they can host. If you
// use a dockspace, submit it early in your app.
...
IMGUI_API void SetNextWindowDockID(
        ImGuiID dock_id,
        ImGuiCond cond = 0); // set next window dock id (FIXME-DOCK)
IMGUI_API void SetNextWindowClass(
        const ImGuiWindowClass*
                window_class); // set next window class (rare/advanced uses:
                               // provide hints to the platform backend via
...
IMGUI_API bool SetDragDropPayload(
        ...
        ImGuiCond cond =
                0); // type is a user defined string of maximum 32 characters.
                    ...
                    // internal types. Data is copied and held by imgui.
...
IMGUI_API bool IsItemClicked(
        ImGuiMouseButton mouse_button =
                0); // is the last item hovered and mouse clicked on? (**)  ==
                    ...
                    // (**) this it NOT equivalent to the behavior of e.g.
...
IMGUI_API bool IsItemDeactivatedAfterEdit(); // was the last item just made
                                             // inactive and made a value change
                                             // when it was active? (e.g.
                                             // Slider/Drag moved). Useful for
                                             // Undo/Redo patterns with widgets
                                             // that requires continuous editing.
                                             // Note that you may get false
                                             // positives (some widgets such as
                                             // Combo()/ListBox()/Selectable()
                                             // will return true even when
                                             // clicking an already selected
                                             // item).
IMGUI_API bool IsItemToggledOpen(); // was the last item open state toggled? set
                                    // by TreeNode().
...
IMGUI_API int GetFrameCount(); // get global imgui frame count. incremented by 1
                               // every frame.
IMGUI_API ImDrawList* GetBackgroundDrawList(); // get background draw list for
                                               // the viewport associated to the
                                               // current window. this draw list
                                               // will be the first rendering
                                               // one. Useful to quickly draw
                                               // shapes/text behind dear imgui
                                               // contents.
IMGUI_API ImDrawList* GetForegroundDrawList(); // get foreground draw list for
                                               // the viewport associated to the
                                               // current window. this draw list
                                               // will be the last rendered one.
                                               // Useful to quickly draw
                                               // shapes/text over dear imgui
                                               // contents.
IMGUI_API ImDrawList* GetBackgroundDrawList(
        ImGuiViewport* viewport); // get background draw list for the given
                                  // viewport. this draw list will be the first
                                  // rendering one. Useful to quickly draw
                                  // shapes/text behind dear imgui contents.
IMGUI_API ImDrawList* GetForegroundDrawList(
        ImGuiViewport* viewport); // get foreground draw list for the given
                                  // viewport. this draw list will be the last
...
IMGUI_API void CaptureKeyboardFromApp(
        bool want_capture_keyboard_value =
                true); // attention: misleading name! manually override
                       ...
                       // entirely left for your application to handle). e.g.
                       // force capture keyboard when your widget is being
...
IMGUI_API bool IsMouseDoubleClicked(
        ImGuiMouseButton button); // did mouse button double-clicked? (note that
...
IMGUI_API bool IsMouseHoveringRect(
        ...
        bool clip = true); // is mouse hovering given bounding rect (in screen
                           ...
                           // disregarding of other consideration of
...
IMGUI_API bool IsMouseDragging(
        ...
        float lock_threshold =
                -1.0f); // is mouse dragging? (if lock_threshold < -1.0f, uses
                        // io.MouseDraggingThreshold)
...
IMGUI_API void CaptureMouseFromApp(
        bool want_capture_mouse_value =
                true); // attention: misleading name! manually override
                       // io.WantCaptureMouse flag next frame (said flag is
                       // entirely left for your application to handle). This is
                       // equivalent to setting "io.WantCaptureMouse =
                       // want_capture_mouse_value;" after the next NewFrame()
                       // call.
...
IMGUI_API void LoadIniSettingsFromMemory(
        ...
        size_t ini_size = 0); // call after CreateContext() and before the first
                              // call to NewFrame() to provide .ini data from
...
// current window.
IMGUI_API ImGuiPlatformIO& GetPlatformIO(); // platform/renderer functions, for
                                            // backend to setup + viewports list.
IMGUI_API void UpdatePlatformWindows(); // call in main loop. will call
                                        // CreateWindow/ResizeWindow/etc.
                                        // platform functions for each secondary
                                        // viewport, and DestroyWindow for each
                                        // inactive viewport.
IMGUI_API void RenderPlatformWindowsDefault(
        ...
        void* renderer_render_arg =
                NULL); // call in main loop. will call RenderWindow/SwapBuffers
                       ...
                       // doesn't have the ImGuiViewportFlags_Minimized flag
                       // set. May be reimplemented by user for custom rendering
                       // needs.
IMGUI_API void DestroyPlatformWindows(); // call DestroyWindow platform
                                         // functions for all viewports. call
                                         // from backend Shutdown() if you need
                                         // to close platform windows before
                                         // imgui shutdown. otherwise will be
                                         // called by DestroyContext().
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoCollapse =
            1 << 5, // Disable user collapsing window by double-clicking on it.
                    // Also referred to as "window menu button" within a docking
                    // node.
...
// Obsolete names (will be removed soon)
    ...
    ImGuiInputTextFlags_AlwaysInsertMode =
            ImGuiInputTextFlags_AlwaysOverwrite // [renamed in 1.82] name was
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoSavedSettings =
            1 << 4, // Disable persisting columns order, width and sort settings
                    // in the .ini file.
    ImGuiTableFlags_ContextMenuInBody =
    ...
    ImGuiTableFlags_NoBordersInBody =
            1
            << 11, // [ALPHA] Disable vertical borders in columns Body (borders
    ...
    ImGuiTableFlags_NoBordersInBodyUntilResize =
            1 << 12, // [ALPHA] Disable vertical borders in columns Body until
                     ...
                     // Headers). -> May move to style Sizing Policy (read above
                     // for defaults)
    ...
    ImGuiTableFlags_NoHostExtendY =
            1 << 17, // Make outer height stop exactly at outer_size.y (prevent
                     // auto-extending table past the limit). Only available
    ...
    ImGuiTableFlags_NoKeepColumnsVisible =
            1 << 18, // Disable keeping column always minimally visible when
                     // ScrollX is off and table gets too small. Not recommended
                     // if columns are resizable.
    ImGuiTableFlags_PreciseWidths =
            1 << 19, // Disable distributing remainder width to stretched
                     ...
                     // appear to be less smooth. Clipping
    ...
    ImGuiTableFlags_PadOuterX =
            1 << 21, // Default if BordersOuterV is on. Enable outer-most
                     // padding. Generally desirable if you have headers.
    ImGuiTableFlags_NoPadOuterX = 1 << 22, // Default if BordersOuterV is off.
    ...
    ImGuiTableFlags_ScrollY =
            1
            << 25, // Enable vertical scrolling. Require 'outer_size' parameter
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_NoClip =
            1 << 7, // Disable clipping for this column (all NoClip columns will
                    // render in a same draw command).
    ImGuiTableColumnFlags_NoSort =
            1 << 8, // Disable ability to sort on this field (even if
    ...
    ImGuiTableColumnFlags_NoHeaderWidth =
            1 << 11, // Disable header text width contribution to automatic
                     // column width.
    ImGuiTableColumnFlags_PreferSortAscending =
            1 << 12, // Make the initial sort direction Ascending when first
                     // sorting on this column (default).
    ImGuiTableColumnFlags_PreferSortDescending =
            1 << 13, // Make the initial sort direction Descending when first
                     // sorting on this column.
    ImGuiTableColumnFlags_IndentEnable =
            1 << 14, // Use current Indent value when entering cell (default for
                     // column 0).
    ImGuiTableColumnFlags_IndentDisable =
            1 << 15, // Ignore current Indent value when entering cell (default
    ...
    ImGuiTableColumnFlags_IsVisible = 1
                                      << 21, // Status: is visible == is enabled
                                             // AND not clipped by scrolling.
...
// Obsolete names (will be removed soon)
                    ...
                    // ImGuiTableColumnFlags_WidthFixed |
                    // ImGuiTableColumnFlags_NoResize, // Column will not
                    // stretch and keep resizing based on submitted contents.
#endif
};
...
enum ImGuiTableBgTarget_
{
    ...
    ImGuiTableBgTarget_RowBg1 = 2, // Set row background color 1 (generally used
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_AnyWindow = 1 << 2, // IsWindowHovered() only: Return true
                                          // if any window is hovered
...
enum ImGuiDockNodeFlags_
{
    ...
    ImGuiDockNodeFlags_KeepAliveOnly =
            1 << 0, // Shared       // Don't display the dockspace node but keep
                    // it alive. Windows docked into this dockspace node won't
                    // be undocked.
    // ImGuiDockNodeFlags_NoCentralNode              = 1 << 1,   // Shared //
    // Disable Central Node (the node which can stay empty)
    ImGuiDockNodeFlags_NoDockingInCentralNode =
            1 << 2, // Shared       // Disable docking inside the Central Node,
                    // which will be always kept empty.
    ImGuiDockNodeFlags_PassthruCentralNode =
            1 << 3, // Shared       // Enable passthru dockspace: 1) DockSpace()
                    // will render a ImGuiCol_WindowBg background covering
                    // everything excepted the Central Node when empty. Meaning
                    // the host window should probably use
                    // SetNextWindowBgAlpha(0.0f) prior to Begin() when using
                    // this. 2) When Central Node is empty: let inputs
                    // pass-through + won't display a DockingEmptyBg background.
                    // See demo for details.
    ImGuiDockNodeFlags_NoSplit =
            1 << 4, // Shared/Local // Disable splitting the node into smaller
                    // nodes. Useful e.g. when embedding dockspaces into a main
                    // root one (the root one may have splitting disabled to
                    // reduce confusion). Note: when turned off, existing splits
                    // will be preserved.
    ImGuiDockNodeFlags_NoResize =
            1 << 5, // Shared/Local // Disable resizing node using the
                    // splitter/separators. Useful with programatically setup
                    // dockspaces.
    ImGuiDockNodeFlags_AutoHideTabBar =
            1 << 6 // Shared/Local // Tab bar will automatically hide when there
                   // is a single window in the dock node.
};
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_DpiEnableScaleViewports =
            1 << 14, // [BETA: Don't use] FIXME-DPI: Reposition and resize imgui
                     // windows when the DpiScale of a viewport changed (mostly
                     // useful for the main viewport hosting other window). Note
                     // that resizing the main window itself is up to your
                     // application.
    ImGuiConfigFlags_DpiEnableScaleFonts =
            1
            << 15, // [BETA: Don't use] FIXME-DPI: Request bitmap-scaled fonts
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_HasMouseHoveredViewport =
            1
            << 11, // Backend Platform supports setting io.MouseHoveredViewport
                   // to the viewport directly under the mouse _IGNORING_
                   // viewports with the ImGuiViewportFlags_NoInputs flag and
                   // _REGARDLESS_ of whether another viewport is focused and
                   // may be capturing the mouse. This information is _NOT EASY_
                   // to provide correctly with most high-level engines! Don't
                   // set this without studying _carefully_ how the backends
                   // handle ImGuiViewportFlags_NoInputs!
...
struct ImVector
{
    ...
    {
        IM_ASSERT(
                it >= Data && it < Data + Size && it_last > it
                && it_last <= Data + Size);
...
struct ImGuiStyle
{
    ...
    ImVec2 SelectableTextAlign; // Alignment of selectable text. Defaults to
                                ...
                                // generally important to keep this left-aligned
                                // if you want to lay multiple items on a same
                                // line.
    ImVec2 DisplayWindowPadding;   // Window position are clamped to be visible
                                   // within the display area or monitors by at
                                   // least this amount. Only applies to regular
                                   // windows.
...
struct ImGuiIO
{
    ...
    bool ConfigWindowsMoveFromTitleBarOnly; // = false       // Enable allowing
                                            ...
                                            // clicking on their title bar. Does
                                            // not apply to windows without a
                                            // title bar.
    ...
    // private clipboard. Override to access OS clipboard on other architectures)
    ...
    float MouseWheelH; // Mouse wheel Horizontal. Most users don't have a mouse
                       ...
                       // backends.
    ImGuiID MouseHoveredViewport; // (Optional) When using multiple viewports:
                                  // viewport the OS mouse cursor is hovering
                                  // _IGNORING_ viewports with the
                                  // ImGuiViewportFlags_NoInputs flag, and
                                  // _REGARDLESS_ of whether another viewport is
                                  // focused. Set io.BackendFlags |=
                                  // ImGuiBackendFlags_HasMouseHoveredViewport
                                  // if you can provide this info. If you don't
                                  // imgui will infer the value using the
                                  // rectangles and last focused time of the
                                  // viewports it knows about (ignoring other OS
                                  // windows).
    ...
    ImGuiKeyModFlags KeyMods; // Key mods flags (same as
                              // io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged
                              // into flags), updated by NewFrame()
...
struct ImGuiWindowClass
{
    ...
    ImGuiID ParentViewportId; // Hint for the platform backend. If non-zero, the
                              // platform backend can create a parent<>child
                              // relationship between the platform windows. Not
                              // conforming backends are free to e.g. parent
                              // every viewport to the main viewport or not.
    ImGuiViewportFlags
            ViewportFlagsOverrideSet; // Viewport flags to set when a window of
                                      // this class owns a viewport. This allows
                                      // you to enforce OS decoration or task
                                      // bar icon, override the defaults on a
                                      // per-window basis.
    ImGuiViewportFlags
            ViewportFlagsOverrideClear; // Viewport flags to clear when a window
    ...
    ImGuiTabItemFlags
            TabItemFlagsOverrideSet; // [EXPERIMENTAL] TabItem flags to set when
                                     // a window of this class gets submitted
                                     // into a dock node tab bar. May use with
                                     // ImGuiTabItemFlags_Leading or
                                     // ImGuiTabItemFlags_Trailing.
    ImGuiDockNodeFlags
            DockNodeFlagsOverrideSet; // [EXPERIMENTAL] Dock node flags to set
                                      // when a window of this class is hosted
                                      // by a dock node (it doesn't have to be
                                      // selected!)
...
// value, otherwise it will crash trying to call a function at this address. This
// is useful for example if you submitted callbacks which you know have altered
// the render state and you want it to be restored. It is not done by default
// because they are many perfectly useful way of altering render state for imgui
...
struct ImDrawCmd
{
    ...
    unsigned int ElemCount; // 4    // Number of indices (multiple of 3) to be
                            // rendered as triangles. Vertices are stored in the
                            // callee ImDrawList's vtx_buffer[] array, indices
                            // in idx_buffer[].
...
// them or add other fields as needed to simplify integration in your engine. The
// type has to be described within the macro (you can either declare the struct
// or use a typedef). This is because ImVec2/ImU32 are likely not declared a the
// time you'd want to set your type up. NOTE: IMGUI DOESN'T CLEAR THE STRUCTURE
// AND DOESN'T CALL A CONSTRUCTOR SO ANY CUSTOM FIELD WILL BE UNINITIALIZED. IF
// YOU ADD EXTRA FIELDS (SUCH AS A 'Z' COORDINATES) YOU WILL NEED TO CLEAR THEM
...
enum ImDrawFlags_
{
    ...
    ImDrawFlags_RoundCornersTopRight =
            1 << 5, // AddRect(), AddRectFilled(), PathRect(): enable rounding
                    // top-right corner only (when rounding > 0.0f, we default
                    // to all corners). Was 0x02.
    ...
    ImDrawFlags_RoundCornersBottomRight =
            1 << 7, // AddRect(), AddRectFilled(), PathRect(): enable rounding
                    // bottom-right corner only (when rounding > 0.0f, we
                    // default to all corners). Wax 0x08.
    ImDrawFlags_RoundCornersNone =
            1 << 8, // AddRect(), AddRectFilled(), PathRect(): disable rounding
...
// bottom-right is == GetMainViewport()->Pos+Size (generally io.DisplaySize). You
// are totally free to apply whatever transformation matrix to want to the data
// (depending on the use of the transformation you may want to apply it to
...
// the oldest structure exposed by the library! Basically, ImDrawList == CmdList)
struct ImDrawData
{
    ...
    ImVec2 FramebufferScale; // Amount of pixels for each unit of DisplaySize.
                             ...
                             // Retina display.
    ImGuiViewport*
            OwnerViewport; // Viewport carrying the ImDrawData instance, might
...
struct ImFontAtlas
{
    ...
    // SetTexID(). The pitch is always = Width * BytesPerPixels (1 or 4) Building
    // in RGBA32 format is provided for convenience and compatibility, but note
    // that unless you manually manipulate or copy color data into the texture
    // (e.g. when using the AddCustomRect*** api), then the RGB pixels emitted
    ...
    IMGUI_API const ImWchar* GetGlyphRangesVietnamese(); // Default + Vietnamese
                                                         // characters
    ...
    ImVec4 TexUvLines[IM_DRAWLIST_TEX_LINES_WIDTH_MAX + 1]; // UVs for baked
                                                            // anti-aliased lines
...
struct ImFont
{
    ...
    short ConfigDataCount;          // 2     // in  // ~ 1        // Number of
                           ...
                           // Bigger than 1 when merging multiple font sources
                           // into one ImFont.
    ...
    ImU8 Used4kPagesMap
            [(IM_UNICODE_CODEPOINT_MAX + 1) / 4096
             / 8]; // 2 bytes if ImWchar=ImWchar16, 34 bytes if
                   // ImWchar==ImWchar32. Store 1-bit for each block of 4K
                   // codepoints that has one active glyph. This is mainly used
                   // to facilitate iterations across all used codepoints.
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_NoDecoration =
            1 << 3, // Platform Window: Disable platform decorations: title bar,
                    ...
                    // ImGuiConfigFlags_ViewportsDecoration is set we only set
                    // this on popups/tooltips)
    ...
    ImGuiViewportFlags_NoInputs =
            1 << 7, // Platform Window: Make mouse pass through so we can drag
    ...
    ImGuiViewportFlags_Minimized =
            1 << 10, // Platform Window: Window is minimized, can skip render.
                     // When minimized we tend to avoid using the viewport
                     // pos/size for clipping window or testing if they are
                     // contained in the viewport.
    ImGuiViewportFlags_NoAutoMerge =
            1 << 11, // Platform Window: Avoid merging this window into another
...
struct ImGuiViewport
{
    ...
    ImDrawData*
            DrawData; // The ImDrawData corresponding to this viewport. Valid
    ...
    void* PlatformHandleRaw; // void* to hold lower-level, platform-native
                             // window handle (e.g. the HWND) when using an
...
struct ImGuiPlatformIO
{
    ...
    //
    // The functions are designed so we can mix and match 2 imgui_impl_xxxx
    // files, one for the Platform (~window/input handling), one for Renderer.
    // Custom engine backends will often provide both Platform and Renderer
    // interfaces and so may not need to use all functions. Platform functions
    // are typically called before their Renderer counterpart, apart from
    ...
    void (*Platform_SetWindowTitle)(
            ImGuiViewport* vp,
            const char* str); // . . U . .  // Set platform window title (given
                              // an UTF-8 string)
    ...
    void (*Platform_UpdateWindow)(
            ImGuiViewport* vp); // . . U . .  // (Optional) Called by
                                ...
                                // allow the platform backend from doing general
                                // book-keeping every frame.
    void (*Platform_RenderWindow)(
            ImGuiViewport* vp,
            void* render_arg); // . . . R .  // (Optional) Main rendering
                               // (platform side! This is often unused, or just
                               // setting a "current" context for OpenGL
                               // bindings). 'render_arg' is the value passed to
                               // RenderPlatformWindowsDefault().
    ...
    void (*Platform_SetImeInputPos)(
            ImGuiViewport* vp,
            ImVec2 pos); // . F . . .  // (Optional) Set IME (Input Method
                         // Editor, e.g. for Asian languages) input position, so
                         // text preview appears over the imgui input box.
                         // FIXME: The call timing of this is inconsistent
    ...
    void (*Renderer_RenderWindow)(
            ImGuiViewport* vp,
            void* render_arg); // . . . R .  // (Optional) Clear framebuffer,
...
struct ImGuiPlatformMonitor
{
    ...
    ImVec2 WorkPos,
            WorkSize; // Coordinates without task bars / side bars / menu bars.
                      ...
                      // region. If you don't have this info, please copy the
...
IMGUI_API bool ListBoxHeader(
        const char* label,
        int items_count,
        int height_in_items = -1); // Helper to calculate size from items_count
                                   // and height_in_items
...
enum ImDrawCornerFlags_
{
    ImDrawCornerFlags_None =
            ImDrawFlags_RoundCornersNone, // Was == 0 prior to 1.82, this is now
                                          // == ImDrawFlags_RoundCornersNone
                                          // which is != 0 and not implicit
    ImDrawCornerFlags_TopLeft =
            ImDrawFlags_RoundCornersTopLeft, // Was == 0x01 (1 << 0) prior
                                             // to 1.82. Order matches
                                             // ImDrawFlags_NoRoundCorner* flag

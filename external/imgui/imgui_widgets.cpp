...
#pragma clang diagnostic ignored                                               \
        "-Wenum-enum-conversion" // warning: bitwise operation between different
                                 ...
                                 // 'XXXFlagsPrivate_')
#pragma clang diagnostic ignored                                               \
        "-Wdeprecated-enum-enum-conversion" // warning: bitwise operation
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    if(label_size.x > 0.0f)
        RenderText(
                ImVec2(value_bb.Max.x + style.ItemInnerSpacing.x,
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    RenderBullet(
            ...
            bb.Min
                    + ImVec2(
                            style.FramePadding.x + g.FontSize * 0.5f,
                            g.FontSize * 0.5f),
            text_col);
    RenderText(
            bb.Min + ImVec2(g.FontSize + style.FramePadding.x * 2, 0.0f),
            text_begin,
            text_end,
            false);
}
                ...
                if((release_in || release_anywhere) && !g.DragDropActive)
                {
                    ...
                    bool is_repeating_already =
                            ...
                            && g.IO.MouseDownDurationPrev[mouse_button]
                                       >= g.IO.KeyRepeatDelay; // Repeat mode
                                                               // trumps <on
                                                               // release>
        ...
        const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if((flags & ImGuiButtonFlags_AlignTextBaseLine)
       && style.FramePadding.y
                  < window->DC.CurrLineTextBaseOffset) // Try to vertically
                                                       // align buttons that are
                                                       // smaller/have no
                                                       // padding so that text
                                                       // baseline matches (bit
                                                       // hacky, since it
                                                       // shouldn't be a flag)
        ...
        ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
    ...
    ImVec2 off =
            dock_node ? ImVec2(
                    IM_FLOOR(-g.Style.ItemInnerSpacing.x * 0.5f) + 0.5f, 0.0f)
                      : ImVec2(0.0f, 0.0f);
        ...
        const ImVec4& tint_col)
{
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2(
                                   (float)frame_padding, (float)frame_padding)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImVec2 label_pos =
            ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x,
        ...
        float hover_visibility_delay)
{
    ...
    ImRect bb_interact = bb;
    bb_interact.Expand(
            axis == ImGuiAxis_Y ? ImVec2(0.0f, hover_extend)
        ...
        const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    {
        ImU32 bg_col = GetColorU32(
                (popup_open || hovered) ? ImGuiCol_ButtonHovered
                                        : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(
                ImVec2(value_x2, frame_bb.Min.y),
                frame_bb.Max,
                bg_col,
                style.FrameRounding,
    ...
    {
        EndPopup();
        IM_ASSERT(0); // This should never happen as we tested for IsPopupOpen()
                      // above
        return false;
    }
    return true;
...
static void SanitizeFormatString(
        const char* fmt, char* fmt_out, size_t fmt_out_size)
{
        ...
        ImGuiSliderFlags flags)
{
    ...
    if(format == NULL)
        format = DataTypeGetInfo(data_type)->PrintFmt;
            ...
            && strcmp(format, "%d")
                       != 0) // (FIXME-LEGACY: Patch old "%.0f" format string to
    ...
    if(power != 1.0f)
    {
        ...
        IM_ASSERT(
                ...
                && p_max != NULL); // When using a power curve the drag needs to
                                   // have known bounds
        drag_flags |= ImGuiSliderFlags_Logarithmic; // Fallback for
                                                    // non-asserting paths
    }
    return DragScalar(
                    ...
                    / ((float)v_max
                       - (float)
                               v_min); // The zero point in parametric space.
                                       ...
                                       // should do (and the most common case of
                                       // a symmetrical range works fine)
        ...
        float zero_deadzone_halfsize)
{
    ...
    if(is_logarithmic)
    {
        ...
        else
        {
            ...
            float t_with_flip =
                    flipped ? (1.0f - t)
                            : t; // t, but flipped if necessary to account for
               ...
               < 0.0f) // Range crosses zero, so we have to do this in two parts
            {
                float zero_point_center =
                        ...
                        / ImAbs((float)v_max
                                - (float)v_min); // The zero point in parametric
                                                 // space
                float zero_point_snap_L =
                        zero_point_center - zero_deadzone_halfsize;
                float zero_point_snap_R =
        ...
        ImRect* out_grab_bb)
{
    ...
    if(g.ActiveId == id)
    {
        ...
        else if(g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            {
                const int decimal_precision =
                        is_floating_point ? ImParseFormatPrecision(format, 3)
                                          : 0;
                if(decimal_precision > 0)
                {
        ...
        ImGuiSliderFlags flags)
{
    ...
    bool value_changed = SliderFloat(
            label, &v_deg, v_degrees_min, v_degrees_max, format, flags);
        ...
        float power)
{
    ...
    if(power != 1.0f)
    {
        IM_ASSERT(
                ...
                && "Call function with ImGuiSliderFlags_Logarithmic flags "
                   "instead of using the old 'float power' function!");
        slider_flags |= ImGuiSliderFlags_Logarithmic; // Fallback for
                                                      // non-asserting paths
    }
    return SliderScalar(
        ...
        const void* p_clamp_max)
{
    ...
    char fmt_buf[32];
    char data_buf[32];
    format = ImParseFormatTrimDecorations(
            format, fmt_buf, IM_ARRAYSIZE(fmt_buf));
    DataTypeFormatString(
        ...
        void* callback_user_data)
{
    ...
    IM_ASSERT(
            !((flags & ImGuiInputTextFlags_CallbackCompletion)
              && (flags
                  & ImGuiInputTextFlags_AllowTabInput))); // Can't use both
                                                          // together (they both
                                                          // use tab key)
    ...
    const ImVec2 frame_size = CalcItemSize(
            ...
            (is_multiline ? g.FontSize * 8.0f : label_size.y)
                    + style.FramePadding.y * 2.0f); // Arbitrary default of 8
                                                    // lines high for multi-line
    ...
    if((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        state->TextAIsValid = false; // TextA is not valid yet (we will display
                                     // buf until then)
        state->CurLenW = ImTextStrFromUtf8(
                state->TextW.Data, buf_size, buf, NULL, &buf_end);
        state->CurLenA =
                (int)(buf_end
                      - buf); // We can't get the result from ImStrncpy() above
    ...
    if(g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        IM_ASSERT(
                ...
                && "Mismatching io.KeyCtrl/io.KeyShift/io.KeyAlt/io.KeySuper "
                   "vs io.KeyMods"); // We rarely do this check, but if anything
                                     // let's do it here.
        ...
        const bool is_osx_shift_shortcut =
                is_osx
                && (io.KeyMods
                    == (ImGuiKeyModFlags_Super | ImGuiKeyModFlags_Shift));
               ...
               != 0)
            {
                ...
                if(event_flag)
                {
                    ...
                    if(buf_dirty)
                    {
                        ...
                        state->CurLenA =
                                callback_data
                                        .BufTextLen; // Assume correct length
    ...
    const ImVec4 clip_rect(
            ...
            frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we
                                            // have adjusted size
        ...
        const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__DisplayMask)); // Check that only 1 is
                                                        // selected
    IM_ASSERT(ImIsPowerOfTwo(
            flags
        ...
        const float* ref_col)
{
    ...
    if(!(flags & ImGuiColorEditFlags__InputMask))
        flags |= ((g.ColorEditOptions & ImGuiColorEditFlags__InputMask)
                 ...
                 & ImGuiColorEditFlags__InputMask;
    IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__PickerMask)); // Check that only 1 is
                                                       // selected
    IM_ASSERT(ImIsPowerOfTwo(
            flags
            & ImGuiColorEditFlags__InputMask)); // Check that only 1 is selected
    ...
    {
        PushItemWidth(
                (alpha_bar ? bar1_pos_x : bar0_pos_x) + bars_width
                - picker_pos.x);
        ImGuiColorEditFlags sub_flags_to_forward =
                ImGuiColorEditFlags__DataTypeMask
                | ImGuiColorEditFlags__InputMask | ImGuiColorEditFlags_HDR
        ...
        sv_cursor_pos.x =
                ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size),
                        ...
                        picker_pos.x + sv_picker_size
                                - 2); // Sneakily prevent the circle to stick
                                      // out too much
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags)
{
    ...
    IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__PickerMask)); // Check only 1 option is
                                                       // selected
    IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__InputMask)); // Check only 1 option is
                                                      // selected
        ...
        const char* label, int items_count, int height_in_items)
{
    ...
    float height_in_items_f =
            (height_in_items < 0 ? ImMin(items_count, 7) : height_in_items)
            + 0.25f;
    ImVec2 size;
    size.x = 0.0f;
...
void ImGui::EndMenuBar()
{
           ...
           && g.NavMoveRequestForward == ImGuiNavForward_None)
        {
            ...
            g.NavDisableHighlight =
                    true; // Hide highlight for the current frame so we don't
                          // see the intermediary selection.
    ...
    window->DC.MenuBarOffset.x =
            window->DC.CursorPos.x
            - window->Pos.x; // Save horizontal position so next append can
                             // reuse it. This is kinda equivalent to a
                             // per-layer CursorPos.
        ...
        ImGuiWindowFlags window_flags)
{
    ...
    SetNextWindowViewport(
            viewport->ID); // Enforce viewport so we don't create our own
                           // viewport when ImGuiConfigFlags_ViewportsNoMerge is
                           // set.
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if(window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x += IM_FLOOR(
                style.ItemSpacing.x
                * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added
    ...
    else
    {
        ...
        RenderArrow(
                ...
                pos
                        + ImVec2(
                                window->DC.MenuColumns.Pos[2] + extra_w
                                        + g.FontSize * 0.30f,
                                0.0f),
                text_col,
                ImGuiDir_Right);
    ...
    if(menu_is_open)
    {
        SetNextWindowPos(
                popup_pos,
                ImGuiCond_Always); // Note: this is super misleading! The value
                                   // will serve as reference for
                                   // FindBestWindowPosForPopup(), not actual
                                   // pos.
        menu_is_open = BeginPopupEx(
                id, flags); // menu_is_open can be 'false' when the popup is
                            // completely clipped (e.g. zero size display)
        ...
        const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if(window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        // inside a menu bar, which is a little misleading but may be useful Note
        // that in this situation we render neither the shortcut neither the
        // selected tick mark
        float w = label_size.x;
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
        PushStyleVar(
...
bool ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    return BeginTabBarEx(
            tab_bar, tab_bar_bb, flags | ImGuiTabBarFlags_IsFocused, NULL);
}
    ...
    if((flags & ImGuiTabBarFlags_Reorderable)
       ...
       || (tab_bar->TabsAddedNew && !(flags & ImGuiTabBarFlags_Reorderable)))
        if(tab_bar->Tabs.Size > 1
           && (flags & ImGuiTabBarFlags_DockNode)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for(int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        section->Width +=
                tab->ContentWidth
                + (section_n == curr_section_n ? g.Style.ItemInnerSpacing.x
                                               : 0.0f);
    ...
    if(central_section_is_visible)
        width_excess =
                ImMax(section_1_w
                              - (tab_bar->BarRect.GetWidth() - section_0_w
                                 - section_2_w),
...
// stack.
static ImU32 ImGui::TabBarCalcTabID(
        ImGuiTabBar* tab_bar, const char* label, ImGuiWindow* docked_window)
{
...
{
    scrolling = ImMin(
            scrolling, tab_bar->WidthAllTabs - tab_bar->BarRect.GetWidth());
        ...
        ImGuiTabBar* tab_bar, ImGuiID tab_id, ImGuiTabBarSection* sections)
{
    ...
    float margin = g.FontSize
                   * 1.0f; // When to scroll to make Tab N+1 visible always make
        ...
        ImGuiWindow* docked_window)
{
    ...
    if(want_clip_rect)
        PushClipRect(
                ImVec2(ImMax(bb.Min.x, tab_bar->ScrollingRectMinX),
                       bb.Min.y - 1),
           ...
           && !(docked_window->Flags & ImGuiWindowFlags_NoMove))
        {
                               ...
                               //g.IO.KeyShift)
            {
                ...
                float threshold_y =
                        (threshold_base * 1.5f)
                        + ImClamp(
                                (ImFabs(g.IO.MouseDragMaxDistanceAbs[0].x)
                                 - threshold_base * 2.0f)
                                        * 0.20f,
                                0.0f,
                                threshold_base * 4.0f);
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ...
    ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size =
            ImVec2(label_size.x + g.Style.FramePadding.x,

    ...
    if(table->InstanceCurrent == 0)
    {
        if(table->ResizedColumn != -1
           && table->ResizedColumnNextWidth != FLT_MAX)
            TableSetColumnWidth(
                    table->ResizedColumn, table->ResizedColumnNextWidth);
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for(int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // might be able to store a log of cells width, per row, for visible
        // rows, but nav/programmatic scroll would have visible artifacts.)
        ...
        if(visible_n < table->FreezeColumnsCount)
            host_clip_rect.Min.x =
                    ImClamp(column->MaxX + TABLE_BORDER_SIZE,
                            host_clip_rect.Min.x,
...
void ImGui::EndTable()
{
    ...
    {
        const float outer_padding_for_border =
                (table->Flags & ImGuiTableFlags_BordersOuterV)
                        ? TABLE_BORDER_SIZE
                        : 0.0f;
    ...
    else if(table->UserOuterSize.x <= 0.0f)
    {
        ...
        outer_window->DC.IdealMaxPos.x =
                ImMax(outer_window->DC.IdealMaxPos.x,
                      table->OuterRect.Min.x + table->ColumnsAutoFitWidth
                              + decoration_size - table->UserOuterSize.x);
...
// Request enabling/disabling a column (often perceived as "showing/hiding" from
// users point of view) Note that end-user can use the context menu to change
// this themselves (right-click in headers, or right-click in columns body with
// ImGuiTableFlags_ContextMenuInBody) Request will be applied during next layout,
// which happens on the first call to TableNextRow() after BeginTable() For the
// getter you can use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsEnabled)
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    const bool is_visible =
            (bg_y2 >= table->InnerClipRect.Min.y
    ...
    // instead of TableBeginRow() so the list clipper can mark end of row and get
    ...
    if(unfreeze_rows_actual)
    {
        ...
        IM_ASSERT(
                table->Bg2ClipRectForDrawCmd.Min.y
...
void ImGui::TableEndCell(ImGuiTable* table)
{
    ...
    *p_max_pos_x = ImMax(*p_max_pos_x, window->DC.CursorMaxPos.x);
    table->RowPosY2 = ImMax(
            table->RowPosY2, window->DC.CursorMaxPos.y + table->CellPaddingY);
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n)
{
    ...
    else if((table->Flags & ImGuiTableFlags_NoKeepColumnsVisible) == 0)
    {
        ...
        max_width =
                ...
                - (table->ColumnsEnabledCount - column->IndexWithinEnabledSet
                   - 1) * min_column_distance
                - column->MinX;
            ...
            merge_group_sub_n++)
        {
            ...
            if(!(column->Flags & ImGuiTableColumnFlags_NoClip))
            {
                ...
                if(!has_freeze_v)
                    ...
                    content_max_x =
                            column->ContentMaxXUnfrozen; // Row freeze: use
                                                         // width after freeze
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    table->DrawSplitter.SetCurrentChannel(
            inner_drawlist, TABLE_DRAW_CHANNEL_BG0);
    inner_drawlist->PushClipRect(
            table->Bg0ClipRectForDrawCmd.Min,
...
float ImGui::TableGetHeaderRowHeight()
{
    ...
    for(int column_n = 0; column_n < columns_count; column_n++)
        if(TableGetColumnFlags(column_n) & ImGuiTableColumnFlags_IsEnabled)
            row_height = ImMax(
                    row_height, CalcTextSize(TableGetColumnName(column_n)).y);
...
void ImGui::TableHeader(const char* label)
{
    ...
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(
            table != NULL && "Need to call TableHeader() after BeginTable()!");
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(
            bb, id, &hovered, &held, ImGuiButtonFlags_AllowItemOverlap);
    if(g.ActiveId != id)
        SetItemAllowOverlap();
       ...
       && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        if(column->SortOrder != -1)
        {
            ...
            {
                PushStyleColor(
                        ImGuiCol_Text, GetColorU32(ImGuiCol_Text, 0.70f));
                RenderText(
...
void ImGui::TableOpenContextMenu(int column_n)
{
    ...
    if(column_n == -1
       && table->CurrentColumn
                  != -1) // When called within a column automatically use this
                         // one (for consistency)
        column_n = table->CurrentColumn;
    ...
    if(table->Flags & ImGuiTableFlags_Resizable)
    {
        if(column != NULL)
        {
            const bool can_resize =
                    ...
                    && column->IsEnabled;
            if(MenuItem(
                       "Size column to fit###SizeOne", NULL, false, can_resize))
                TableSetColumnWidthAutoSingle(table, column_n);
        }
        ...
        if(table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount
           && (table->Flags & ImGuiTableFlags_SizingMask_)
            ...
            size_all_desc =
                    "Size all columns to default###SizeAll"; // All stretch or
                                                             // mixed
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for(int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(
                ...
                "Enabled: %d, VisibleX/Y: %d/%d, RequestOutput: %d, SkipItems: "
                "%d, DrawChannels: %d,%d\n"

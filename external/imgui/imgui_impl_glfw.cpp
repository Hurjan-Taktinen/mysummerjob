...
//  e.g. ImGui::IsKeyPressed(GLFW_KEY_SPACE). [X] Platform: Multi-viewport
...
//  2021-XX-XX: Platform: Added support for multiple windows via the
//  ImGuiPlatformIO interface. 2020-01-17: Inputs: Disable error callback while
//  assigning mouse cursors because some X11 setup don't have them and it
//  generates errors. 2019-12-05: Inputs: Added support for new mouse cursors
//  added in GLFW 3.4+ (resizing cursors, not allowed cursor). 2019-10-18: Misc:
//  Previously installed user callbacks are now restored on shutdown. 2019-07-21:
//  Inputs: Added mapping for ImGuiKey_KeyPadEnter. 2019-05-11: Inputs: Don't
//  filter value from character callback before calling AddInputCharacter().
//  2019-03-12: Misc: Preserve DisplayFramebufferScale when main window is
//  minimized. 2018-11-30: Misc: Setting up io.BackendPlatformName so it can be
//  displayed in the About Window. 2018-11-07: Inputs: When installing our GLFW
//  callbacks, we save user's previously installed ones - if any - and chain
//  call them. 2018-08-01: Inputs: Workaround for Emscripten which doesn't seem
//  to handle focus related calls. 2018-06-29: Inputs: Added support for the
...
// Chain GLFW callbacks for main viewport: our callbacks will call the user's
...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
    ...
    for(int n = 0; n < platform_io.Viewports.Size; n++)
    {
        ...
        if(focused)
        {
            ...
            else
            {
                ...
                if(io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
                {
                    ...
                    io.MousePos =
                            ImVec2((float)mouse_x + window_x,
                                   (float)mouse_y + window_y);
                }
                else
                {
                    // Single viewport mode: mouse position in client window
                    // coordinates (io.MousePos is (0,0) when the mouse is on
                    // the upper-left corner of the app window)
        ...
        // to the viewport the OS mouse cursor is hovering. Important: this
        // information is not easy to provide and many high-level windowing
        // library won't be able to provide it correctly, because
        // - This is _ignoring_ viewports with the ImGuiViewportFlags_NoInputs
        // flag (pass-through windows).
        // - This is _regardless_ of whether another viewport is focused or
        // being dragged from. If ImGuiBackendFlags_HasMouseHoveredViewport is
        // not set by the backend, imgui will ignore this field and infer the
        // information by relying on the rectangles and last focused time of
        // every viewports it knows about. It will be unaware of other windows
        // that may be sitting between or over your windows. [GLFW] FIXME: This
        // is currently only correct on Win32. See what we do below with the
...
static void ImGui_ImplGlfw_WindowPosCallback(GLFWwindow* window, int, int)
{
    if(ImGuiViewport* viewport = ImGui::FindViewportByPlatformHandle(window))
    {
        if(ImGuiViewportDataGlfw* data =
                   (ImGuiViewportDataGlfw*)viewport->PlatformUserData)
        {
            bool ignore_event =
                    (ImGui::GetFrameCount()
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport* viewport)
{
    ...
    GLFWwindow* share_window =
            (g_ClientApi == GlfwClientApi_OpenGL) ? g_Window : NULL;
    ...
    glfwSetMouseButtonCallback(
            data->Window, ImGui_ImplGlfw_MouseButtonCallback);
    ...
    glfwSetWindowCloseCallback(
            data->Window, ImGui_ImplGlfw_WindowCloseCallback);
        ...
        WndProcNoInputs(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    if(msg == WM_NCHITTEST)
    {
        // Let mouse pass-through the window. This will allow the backend to set
        // io.MouseHoveredViewport properly (which is OPTIONAL). The
...
static void ImGui_ImplGlfw_SetWindowTitle(
...
// We provide a Win32 implementation because this is such a common issue for IME
// users

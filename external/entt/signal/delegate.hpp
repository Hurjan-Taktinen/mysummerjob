...
template<typename Class, typename Ret, typename... Args, typename... Other>
auto function_pointer(Ret (Class::*)(Args...) const, Other&&...)
        -> Ret (*)(Args...);
...
using function_pointer_t =
        decltype(internal::function_pointer(std::declval<Type>()...));
...
public:
                ...
                std::is_invocable_r_v<Ret, decltype(Candidate), Type&, Args...>)
        {
            fn = [](const void* payload, Args... args) -> Ret {
                ...
                return Ret(std::invoke(
                        Candidate, *curr, std::forward<Args>(args)...));
            };
        }
        else
        {
            fn = wrap<Candidate>(
                    value_or_instance,
                    internal::index_sequence_for(internal::function_pointer_t<
                                                 decltype(Candidate),
                                                 Type>{}));
        }
    }
     ...
     */
    [[nodiscard]] const void* instance() const ENTT_NOEXCEPT { return data; }
     ...
     */
    [[nodiscard]] bool operator==(const delegate<Ret(Args...)>& other) const
            ENTT_NOEXCEPT
    {
...
[[nodiscard]] bool operator!=(
        ...
        const delegate<Ret(Args...)>& rhs) ENTT_NOEXCEPT
{
    return !(lhs == rhs);
}
 ...
 */
template<auto Candidate>
delegate(connect_arg_t<Candidate>) -> delegate<std::remove_pointer_t<
        internal::function_pointer_t<decltype(Candidate)>>>;

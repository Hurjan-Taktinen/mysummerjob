...
[[nodiscard]] bool find_if(const Node* candidate, const Node* node)
        ENTT_NOEXCEPT
{
...
[[nodiscard]] bool find_if_not(const Id id, Node* node, const Node* owner)
        ENTT_NOEXCEPT
{
    if constexpr(std::is_pointer_v<Id>)
    {
        return node
               && ((*node->id == *id && node != owner)
                   || find_if_not(id, node->next, owner));
    }
    else
    {
        return node
...
private:
    ...
    void unpack(
            ...
            std::tuple<Property...> property,
            Other&&... other)
    {
        unroll<Step>(
                choice<3>,
                std::move(std::get<Index>(property))...,
    ...
    {
        unpack<Step>(
                std::index_sequence_for<Property...>{},
                std::move(property),
                std::forward<Other>(other)...);
    }
    ...
    template<std::size_t Step = 0, typename Property, typename... Other>
    std::enable_if_t<!std::is_invocable_v<Property>> unroll(
            choice_t<1>, Property&& property, Other&&... other)
    {
    ...
    void assign(Key&& key, meta_any value = {})
    {
        ...
        static internal::meta_prop_node node{
                nullptr, property[0u], property[1u]};
...
public:
     ...
     */
    meta_factory(internal::meta_prop_node** target) ENTT_NOEXCEPT : curr{target}
    {
    }
    ...
    auto prop(PropertyOrKey&& property_or_key, Value&&... value) &&
    {
        ...
        {
            assign(std::forward<PropertyOrKey>(property_or_key),
...
struct meta_factory<Type>
{
    ...
    auto type(const id_type id = type_hash<Type>::value())
    {
        ...
        ENTT_ASSERT(!internal::find_if_not(
                id, *internal::meta_context::global(), node));
        node->id = id;
        ...
        static internal::meta_base_node node{
                ...
                [](const void* instance) ENTT_NOEXCEPT -> const void* {
                    return static_cast<const Base*>(
                            static_cast<const Type*>(instance));
                }};
    ...
    {
        static_assert(
                std::is_convertible_v<Type, To>,
                "Could not convert to the required type");
        auto* const type = internal::meta_info<Type>::resolve();
    ...
    auto conv() ENTT_NOEXCEPT
    {
        ...
        static internal::meta_conv_node node{
                ...
                [](const void* instance) -> meta_any {
                    return std::invoke(
                            Candidate, *static_cast<const Type*>(instance));
                }};
    ...
    auto ctor() ENTT_NOEXCEPT
    {
        ...
        static_assert(
                std::is_same_v<
                        std::decay_t<typename descriptor::return_type>,
                        Type>,
        ...
        static internal::meta_ctor_node node{
                ...
                [](const typename internal::meta_ctor_node::size_type index)
                        ENTT_NOEXCEPT {
                            return meta_arg(
                                    typename descriptor::args_type{}, index);
                        },
                [](meta_any* const args) {
                    return meta_invoke<Type, Candidate, Policy>(
                            {},
                            args,
    ...
    auto ctor() ENTT_NOEXCEPT
    {
        ...
        static internal::meta_ctor_node node{
                ...
                [](meta_any* const args) {
                    return meta_construct<Type, Args...>(
                            args,
                            std::make_index_sequence<
                                    descriptor::args_type::size>{});
                }};
    ...
    {
        static_assert(
                std::is_invocable_v<decltype(Func), Type&>,
    ...
    auto data(const id_type id) ENTT_NOEXCEPT
    {
        ...
        else
        {
            ...
            return meta_factory<
                    Type,
                    std::integral_constant<decltype(Data), Data>>{&node.prop};
        }
    }
    ...
    auto data(const id_type id) ENTT_NOEXCEPT
    {
        using underlying_type = std::remove_reference_t<
                std::invoke_result_t<decltype(Getter), Type&>>;
        auto* const type = internal::meta_info<Type>::resolve();
        ...
        static internal::meta_data_node node{
                ...
                std::is_same_v<
                        decltype(Setter),
                        std::nullptr_t> || (std::is_member_object_pointer_v<decltype(Setter)> && std::is_const_v<underlying_type>),
                false,
        ...
        return meta_factory<
                Type,
                std::integral_constant<decltype(Setter), Setter>,
    ...
    auto func(const id_type id) ENTT_NOEXCEPT
    {
        ...
        static internal::meta_func_node node{
                ...
                &internal::meta_info<std::conditional_t<
                        std::is_same_v<Policy, as_void_t>,
                        void,
                        typename descriptor::return_type>>::resolve,
                [](const typename internal::meta_func_node::size_type index)
                        ENTT_NOEXCEPT {
                            return meta_arg(
                                    typename descriptor::args_type{}, index);
                        },
        ...
        for(; *it && (*it)->id != id; it = &(*it)->next)
            ;
        for(; *it && (*it)->id == id && (*it)->arity < node.arity;
            it = &(*it)->next)
            ;

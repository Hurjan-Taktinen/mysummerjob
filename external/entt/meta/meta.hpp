...
public:
    ...
    meta_sequence_container(std::in_place_type_t<Type>, any instance)
            ENTT_NOEXCEPT :
...
public:
     ...
     */
    template<typename Type>
    meta_associative_container(std::in_place_type_t<Type>, any instance)
            ENTT_NOEXCEPT :
...
class meta_any
{
    ...
    static void basic_vtable(
            const operation op,
            [[maybe_unused]] const any& from,
            [[maybe_unused]] void* to)
    {
        if constexpr(!std::is_void_v<Type>)
        {
            ...
            case operation::DTOR:
                ...
                {
                    if(auto* curr = static_cast<internal::meta_type_node*>(to);
                       curr->dtor)
                    {
            ...
            case operation::REF:
            case operation::CREF:
                *static_cast<meta_any*>(to) =
                        (op == operation::REF
                                 ? meta_any{std::ref(any_cast<Type&>(
                                         const_cast<any&>(from)))}
                                 : meta_any{std::cref(
                                         any_cast<const std::decay_t<Type>&>(
                                                 from))});
                break;
            ...
            case operation::CDEREF:
                ...
                {
                    using element_type =
                            std::remove_const_t<typename std::pointer_traits<
                                    std::decay_t<Type>>::element_type>;

                    if constexpr(std::is_function_v<element_type>)
                    {
                        *static_cast<meta_any*>(to) =
                                any_cast<std::decay_t<Type>>(from);
                    }
                    else if constexpr(!std::is_same_v<element_type, void>)
                    {
                        using adl_meta_pointer_like_type =
                                adl_meta_pointer_like<std::decay_t<Type>>;

                        if constexpr(
                                std::is_lvalue_reference_v<
                                        decltype(adl_meta_pointer_like_type::dereference(
                                                std::declval<const std::decay_t<
                                                        Type>&>()))>)
                        {
                            auto&& obj =
                                    adl_meta_pointer_like_type::dereference(
                                            any_cast<const std::decay_t<Type>&>(
                                                    from));
                            *static_cast<meta_any*>(to) =
                                    (op == operation::DEREF
                                             ? meta_any{std::ref(obj)}
                                             : meta_any{std::cref(obj)});
                        }
                        else
                        {
                            *static_cast<meta_any*>(to) =
            ...
            case operation::SEQ:
            case operation::CSEQ:
                if constexpr(is_complete_v<meta_sequence_container_traits<
                                     std::decay_t<Type>>>)
                {
                    *static_cast<meta_sequence_container*>(to) = {
                            std::in_place_type<std::decay_t<Type>>,
                            (op == operation::SEQ
                                     ? const_cast<any&>(from).as_ref()
                                     : from.as_ref())};
                }
                break;
            case operation::ASSOC:
            case operation::CASSOC:
                if constexpr(is_complete_v<meta_associative_container_traits<
                                     std::decay_t<Type>>>)
                {
                    *static_cast<meta_associative_container*>(to) = {
                            std::in_place_type<std::decay_t<Type>>,
                            (op == operation::ASSOC
                                     ? const_cast<any&>(from).as_ref()
                                     : from.as_ref())};
...
public:
    /*! @brief Default constructor. */
    meta_any() ENTT_NOEXCEPT : storage{}, node{}, vtable{&basic_vtable<void>} {}
    ...
    template<
            ...
            typename = std::enable_if_t<
                    !std::is_same_v<std::decay_t<Type>, meta_any>>>
    meta_any(Type&& value) :
        meta_any{
                std::in_place_type<std::decay_t<Type>>,
    ...
    ~meta_any() { vtable(operation::DTOR, storage, node); }
    ...
    {
        std::exchange(vtable, std::exchange(other.vtable, &basic_vtable<void>))(
                operation::DTOR, storage, node);
        storage = std::move(other.storage);
        node = std::exchange(other.node, nullptr);
        return *this;
    }
     ...
     */
    template<typename Type>
    std::enable_if_t<!std::is_same_v<std::decay_t<Type>, meta_any>, meta_any&> operator=(
            Type&& value)
    {
    ...
    /*! @copydoc data */
    [[nodiscard]] void* data() ENTT_NOEXCEPT { return storage.data(); }
    ...
    [[nodiscard]] const Type* try_cast() const
    {
        if(node)
        {
            ...
            else if(const auto* base = internal::meta_visit<
                            &internal::meta_type_node::base>(
                            [info](const auto* curr) {
        ...
        if(node)
        {
                    ...
                    base)
            {
                return static_cast<Type*>(
                        const_cast<constness_as_t<void, Type>*>(base->cast(
                                static_cast<constness_as_t<any, Type>&>(storage)
                                        .data())));
            }
        }
    ...
    [[nodiscard]] Type cast()
    {
        // forces const on non-reference types to make them work also with
        // wrappers for const references
    ...
    [[nodiscard]] meta_any allow_cast() const
    {
        ...
        {
            if(const auto* const conv =
                       internal::meta_visit<&internal::meta_type_node::conv>(
                               [info = type_id<Type>()](const auto* curr) {
                                   return curr->type()->info == info;
                               },
                               node);
               conv)
    ...
    {
        std::exchange(vtable, &basic_vtable<Type>)(
                operation::DTOR, storage, node);
    ...
    void reset()
    {
        std::exchange(vtable, &basic_vtable<void>)(
                operation::DTOR, storage, node);
        storage.reset();
        node = nullptr;
    }
    ...
    [[nodiscard]] meta_sequence_container as_sequence_container() const
            ENTT_NOEXCEPT
    {
        meta_sequence_container proxy;
     ...
     */
    [[nodiscard]] meta_associative_container as_associative_container()
            ENTT_NOEXCEPT
    {
    ...
    [[nodiscard]] meta_associative_container as_associative_container() const
            ENTT_NOEXCEPT
    {
        meta_associative_container proxy;
    ...
    [[nodiscard]] bool operator==(const meta_any& other) const
    {
        return (!node && !other.node)
               || (node && other.node && node->info == other.node->info
                   && storage == other.storage);
    }
 ...
 */
[[nodiscard]] inline bool operator!=(const meta_any& lhs, const meta_any& rhs)
        ENTT_NOEXCEPT
{
    return !(lhs == rhs);
}
...
struct meta_handle
{
    ...
    template<
            typename Type,
            typename = std::enable_if_t<
                    !std::is_same_v<std::decay_t<Type>, meta_handle>>>
...
struct meta_ctor
{
    ...
    meta_ctor(const node_type* curr = nullptr) ENTT_NOEXCEPT : node{curr} {}
     ...
     */
    [[nodiscard]] size_type arity() const ENTT_NOEXCEPT { return node->arity; }
     ...
     */
    [[nodiscard]] meta_any invoke(
            meta_any* const args, const size_type sz) const
    {
    ...
    {
        return internal::meta_visit<&node_type::prop>(
                [&key](const auto* curr) { return curr->id == key; }, node);
    }
...
struct meta_data
{
    ...
    /*! @copydoc meta_prop::meta_prop */
    meta_data(const node_type* curr = nullptr) ENTT_NOEXCEPT : node{curr} {}

    /*! @copydoc meta_type::id */
    [[nodiscard]] id_type id() const ENTT_NOEXCEPT { return node->id; }
     ...
     */
    [[nodiscard]] bool is_const() const ENTT_NOEXCEPT { return node->is_const; }
    ...
    {
        return node->set
               && node->set(std::move(instance), std::forward<Type>(value));
    }
...
struct meta_func
{
    ...
    /*! @copydoc meta_prop::meta_prop */
    meta_func(const node_type* curr = nullptr) ENTT_NOEXCEPT : node{curr} {}
    ...
    {
        return sz == arity() ? node->invoke(std::move(instance), args)
                             : meta_any{};
    }
...
class meta_type
{
    static bool can_cast_or_convert(
    ...
    [[nodiscard]] static const internal::meta_ctor_node* ctor(
            const internal::meta_ctor_node* curr, std::index_sequence<Index...>)
    {
        for(; curr; curr = curr->next)
        {
            if(curr->arity == sizeof...(Args)
               && (can_cast_or_convert(
                           internal::meta_info<Args>::resolve(),
                           curr->arg(Index).info())
                   && ...))
            {
                return curr;
...
public:
    ...
    /*! @copydoc meta_prop::meta_prop */
    meta_type(node_type* curr = nullptr) ENTT_NOEXCEPT : node{curr} {}
     ...
     */
    [[nodiscard]] type_info info() const ENTT_NOEXCEPT { return node->info; }
     ...
     */
    [[nodiscard]] bool is_void() const ENTT_NOEXCEPT { return node->is_void; }
     ...
     */
    [[nodiscard]] bool is_array() const ENTT_NOEXCEPT { return node->is_array; }

    /**
     * @brief Checks whether a type refers to an enum or not.
     * @return True if the underlying type is an enum, false otherwise.
     */
    [[nodiscard]] bool is_enum() const ENTT_NOEXCEPT { return node->is_enum; }

    /**
     * @brief Checks whether a type refers to an union or not.
     * @return True if the underlying type is an union, false otherwise.
     */
    [[nodiscard]] bool is_union() const ENTT_NOEXCEPT { return node->is_union; }

    /**
     * @brief Checks whether a type refers to a class or not.
     * @return True if the underlying type is a class, false otherwise.
     */
    [[nodiscard]] bool is_class() const ENTT_NOEXCEPT { return node->is_class; }
    ...
    {
        return is_template_specialization() ? node->template_info.type()
                                            : meta_type{};
    }
     ...
     */
    [[nodiscard]] inline meta_type template_arg(size_type index) const
            ENTT_NOEXCEPT
    {
        return index < template_arity() ? node->template_info.arg(index)
                                        : meta_type{};
    }
     ...
     */
    [[nodiscard]] size_type rank() const ENTT_NOEXCEPT { return node->rank; }
     ...
     */
    [[nodiscard]] meta_range<meta_type, internal::meta_base_node> base() const
            ENTT_NOEXCEPT
    {
        return node->base;
    }
    ...
    {
        return internal::meta_visit<&node_type::base>(
                [id](const auto* curr) { return curr->type()->id == id; },
                node);
    }
    ...
    {
        return ctor<Args...>(
                node->ctor, std::make_index_sequence<sizeof...(Args)>{});
    }
    ...
    {
        return internal::meta_visit<&node_type::data>(
                [id](const auto* curr) { return curr->id == id; }, node);
    }
    ...
    {
        return internal::meta_visit<&node_type::func>(
                [id](const auto* curr) { return curr->id == id; }, node);
    }
     ...
     * @return A meta any containing the new instance, if any.
     */
    [[nodiscard]] meta_any construct(
            meta_any* const args, const size_type sz) const
    {
        meta_any ret{};
        internal::meta_visit<&node_type::ctor>(
                [args, sz, &ret](const auto* curr) {
                    return (curr->arity == sz) && (ret = curr->invoke(args));
                },
                node);
        return ret;
    }
            ...
            const size_type sz) const
    {
        ...
        for(auto* it = internal::meta_visit<&node_type::func>(
                    [id, sz](const auto* curr) {
                        return curr->id == id && curr->arity == sz;
                    },
                    node);
            it && it->id == id && it->arity == sz;
            it = it->next)
        {
            size_type direct{};
            size_type ext{};
            ...
            for(size_type next{}; next < sz && next == (direct + ext); ++next)
            {
                ...
                type.info() == req
                        ? ++direct
                        : (ext += can_cast_or_convert(type.node, req));
            }
        ...
        return (candidate && !ambiguous)
                       ? candidate->invoke(std::move(instance), args)
                       : meta_any{};
    }
    ...
    template<typename... Args>
    meta_any invoke(
            const id_type id, meta_handle instance, Args&&... args) const
    {
        meta_any arguments[sizeof...(Args) + 1u]{std::forward<Args>(args)...};
    ...
    bool set(const id_type id, meta_handle instance, Type&& value) const
    {
        ...
        return candidate ? candidate.set(
                       std::move(instance), std::forward<Type>(value))
                         : false;
    }
    ...
    void reset() ENTT_NOEXCEPT
    {
        ...
        const auto unregister_all =
                y_combinator{[](auto&& self, auto** curr, auto... member) {
                    while(*curr)
                    {
                        auto* prev = *curr;
                        (self(&(prev->*member)), ...);
 ...
 */
[[nodiscard]] inline bool operator!=(const meta_type& lhs, const meta_type& rhs)
        ENTT_NOEXCEPT
{
    return !(lhs == rhs);
}
...
[[nodiscard]] inline meta_type meta_ctor::arg(size_type index) const
        ENTT_NOEXCEPT
{
...
[[nodiscard]] inline meta_type meta_func::arg(size_type index) const
        ENTT_NOEXCEPT
{
    return index < arity() ? node->arg(index) : meta_type{};
}
...
class meta_sequence_container::meta_iterator
{
    ...
    static void basic_vtable(const operation op, const any& from, void* to)
    {
        switch(op)
        {
        case operation::INCR: ++any_cast<It&>(const_cast<any&>(from)); break;
        case operation::DEREF:
            static_cast<meta_any*>(to)
                    ->emplace<typename std::iterator_traits<It>::reference>(
                            *any_cast<const It&>(from));
            break;
        }
    }
...
public:
     ...
     */
    template<typename It>
    meta_iterator(It iter) : vtable{&basic_vtable<It>}, handle{std::move(iter)}
    {
    }
     ...
     */
    [[nodiscard]] bool operator==(const meta_iterator& other) const
            ENTT_NOEXCEPT
    {
...
struct meta_sequence_container::meta_sequence_container_proxy
{
    ...
    [[nodiscard]] static std::pair<iterator, bool> insert(
            any& container, iterator it, meta_any& value)
    {
        if(auto* const cont = any_cast<Type>(&container); cont)
        {
            // this abomination is necessary because only on macos value_type
            // and const_reference are different types for std::vector<bool>
            if(value.allow_cast<typename Type::const_reference>()
               || value.allow_cast<typename Type::value_type>())
            {
                const auto* element = value.try_cast<std::remove_reference_t<
                        typename Type::const_reference>>();
                auto ret = traits_type::insert(
                        *cont,
                        any_cast<const typename Type::iterator&>(it.handle),
                        element ? *element
                                : value.cast<typename Type::value_type>());
    ...
    [[nodiscard]] static std::pair<iterator, bool> erase(
            any& container, iterator it)
    {
        if(auto* const cont = any_cast<Type>(&container); cont)
        {
            auto ret = traits_type::erase(
                    *cont, any_cast<const typename Type::iterator&>(it.handle));
            return {iterator{std::move(ret.first)}, ret.second};
        }
    ...
    [[nodiscard]] static meta_any get(any& container, size_type pos)
    {
        ...
        {
            return meta_any{
                    std::in_place_type<typename Type::reference>,
                    traits_type::get(*cont, pos)};
        }

        return meta_any{
                std::in_place_type<typename Type::const_reference>,
                traits_type::cget(any_cast<const Type&>(container), pos)};
    }
};
 ...
 */
[[nodiscard]] inline meta_type meta_sequence_container::value_type() const
        ENTT_NOEXCEPT
{
 ...
 */
[[nodiscard]] inline meta_sequence_container::size_type meta_sequence_container::
        size() const ENTT_NOEXCEPT
{
 ...
 */
[[nodiscard]] inline meta_sequence_container::iterator meta_sequence_container::
        begin()
{
 ...
 */
inline std::pair<meta_sequence_container::iterator, bool> meta_sequence_container::
        insert(iterator it, meta_any value)
{
 ...
 */
[[nodiscard]] inline meta_sequence_container::operator bool() const
        ENTT_NOEXCEPT
{
...
class meta_associative_container::meta_iterator
{
    ...
    static void basic_vtable(const operation op, const any& from, void* to)
    {
        ...
        case operation::DEREF:
            ...
            {
                static_cast<std::pair<meta_any, meta_any>*>(to)->first =
                        std::cref(*any_cast<const It&>(from));
            }
            else
            {
                *static_cast<std::pair<meta_any, meta_any>*>(to) =
                        std::make_pair<meta_any, meta_any>(
                                std::cref(any_cast<const It&>(from)->first),
                                std::ref(any_cast<const It&>(from)->second));
            }
            break;
        }
    }
...
public:
    ...
    meta_iterator(std::integral_constant<bool, KeyOnly>, It iter) :
        vtable{&basic_vtable<KeyOnly, It>}, handle{std::move(iter)}
    {
    }
...
struct meta_associative_container::meta_associative_container_proxy
{
    ...
    [[nodiscard]] static iterator begin(any& container)
    {
        if(auto* const cont = any_cast<Type>(&container); cont)
        {
            return iterator{
                    ...
                    traits_type::begin(*cont)};
        }

        return iterator{
                is_key_only_meta_associative_container<Type>{},
                traits_type::cbegin(any_cast<const Type&>(container))};
    }
    ...
    [[nodiscard]] static iterator end(any& container)
    {
        if(auto* const cont = any_cast<Type>(&container); cont)
        {
            return iterator{
                    is_key_only_meta_associative_container<Type>{},
                    traits_type::end(*cont)};
        }

        return iterator{
                is_key_only_meta_associative_container<Type>{},
                traits_type::cend(any_cast<const Type&>(container))};
    }

    [[nodiscard]] static bool insert(
            any& container, meta_any& key, meta_any& value)
    {
        if(auto* const cont = any_cast<Type>(&container);
           cont && key.allow_cast<const typename Type::key_type&>())
        {
            if constexpr(is_key_only_meta_associative_container_v<Type>)
            {
                return traits_type::insert(
                        *cont, key.cast<const typename Type::key_type&>());
            }
            else
            {
                if(value.allow_cast<const typename Type::mapped_type&>())
                {
                    return traits_type::insert(
                            *cont,
                            key.cast<const typename Type::key_type&>(),
 ...
 */
[[nodiscard]] inline bool meta_associative_container::key_only() const
        ENTT_NOEXCEPT
{
 ...
 */
[[nodiscard]] inline meta_type meta_associative_container::key_type() const
        ENTT_NOEXCEPT
{
    return key_type_fn();
}
 ...
 */
[[nodiscard]] inline meta_type meta_associative_container::mapped_type() const
        ENTT_NOEXCEPT
{
...
[[nodiscard]] inline meta_type meta_associative_container::value_type() const
        ENTT_NOEXCEPT
{
    return value_type_fn();
}
...
[[nodiscard]] inline meta_associative_container::size_type meta_associative_container::
        size() const ENTT_NOEXCEPT
{
    return size_fn(storage);
}
...
[[nodiscard]] inline meta_associative_container::iterator meta_associative_container::
        begin()
{
    return begin_fn(storage);
}
 ...
 */
inline bool meta_associative_container::insert(
        meta_any key, meta_any value = {})
{
 ...
 * @return False if the proxy is invalid, true otherwise.
 */
[[nodiscard]] inline meta_associative_container::operator bool() const
        ENTT_NOEXCEPT
{
    return static_cast<bool>(storage);
}

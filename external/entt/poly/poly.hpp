...
class poly_vtable
{
    ...
    static auto vtable_entry(Ret (*)(inspector&, Args...))
            -> Ret (*)(basic_any<Len, Align>&, Args...);

    template<typename Ret, typename... Args>
    static auto vtable_entry(Ret (*)(const inspector&, Args...))
    ...
    template<typename Ret, typename... Args>
    static auto vtable_entry(Ret (inspector::*)(Args...) const)
            -> Ret (*)(const basic_any<Len, Align>&, Args...);
        ...
        else
        {
            entry = +[](Any& instance, Args... args) -> Ret {
                return static_cast<Ret>(std::invoke(
                        Candidate,
                        any_cast<constness_as_t<Type, Any>&>(instance),
    ...
    [[nodiscard]] static auto fill_vtable(std::index_sequence<Index...>)
    {
        ...
        (fill_vtable_entry<
                 ...
                 value_list_element_v<
                         Index,
                         typename Concept::template impl<Type>>>(
                 std::get<Index>(impl)),
         ...);
        return impl;
    }
...
public:
    ...
    [[nodiscard]] static const auto* instance()
    {
        ...
        static const auto vtable = fill_vtable<Type>(
                std::make_index_sequence<Concept::template impl<Type>::size>{});
        return &vtable;
    }
};
...
struct poly_base
{
    ...
    [[nodiscard]] decltype(auto) invoke(
            const poly_base& self, Args&&... args) const
    {
        const auto& poly = static_cast<const Poly&>(self);
        return std::get<Member>(*poly.vtable)(
...
{
    return std::forward<Poly>(self).template invoke<Member>(
...
public:
    ...
    basic_poly() ENTT_NOEXCEPT : storage{}, vtable{} {}
    ...
    explicit basic_poly(std::in_place_type_t<Type>, Args&&... args) :
        ...
        vtable{poly_vtable<Concept, Len, Align>::template instance<
     ...
     */
    basic_poly(basic_poly&& other) ENTT_NOEXCEPT : basic_poly{}
    {
        swap(*this, other);
    }
     ...
     */
    [[nodiscard]] concept_type* operator->() ENTT_NOEXCEPT { return this; }

...
class basic_any
{
    ...
    using vtable_type =
            const void*(const operation, const basic_any&, const void*);

    template<typename Type>
    static constexpr bool in_situ =
            Len && alignof(Type) <= alignof(storage_type)
    ...
    template<typename Type>
    static const void* basic_vtable(
            [[maybe_unused]] const operation op,
            [[maybe_unused]] const basic_any& from,
            [[maybe_unused]] const void* to)
    {
        if constexpr(std::is_void_v<Type>)
        {
            ...
            case operation::MOVE:
            case operation::REF:
            case operation::CREF: as<basic_any>(to).vtable = from.vtable; break;
            default: break;
            }
        }
        else if constexpr(std::is_lvalue_reference_v<Type>)
        {
            ...
            case operation::COPY:
                ...
                {
                    as<basic_any>(to) =
                            *static_cast<const base_type*>(from.instance);
                }
                break;
            case operation::MOVE:
                ...
                [[fallthrough]];
            case operation::DTOR: break;
            case operation::COMP:
                return compare<base_type>(from.instance, to) ? to : nullptr;
            case operation::ADDR:
                return std::is_const_v<std::remove_reference_t<Type>>
                               ? nullptr
                               : from.instance;
            case operation::CADDR: return from.instance;
            case operation::REF:
                as<basic_any>(to).instance = from.instance;
        ...
        else if constexpr(in_situ<Type>)
        {
            ...
            auto* instance = const_cast<Type*>(
                    std::launder(reinterpret_cast<const Type*>(&from.storage)));
#else
            auto* instance = const_cast<Type*>(
                    reinterpret_cast<const Type*>(&from.storage));
#endif
            ...
            case operation::COPY:
                ...
                {
                    new(&as<basic_any>(to).storage)
                            Type{std::as_const(*instance)};
                    as<basic_any>(to).vtable = from.vtable;
                }
                break;
            case operation::MOVE:
                ...
                as<basic_any>(to).vtable = from.vtable;
                break;
            case operation::DTOR: instance->~Type(); break;
            case operation::COMP:
            ...
            case operation::CREF:
                ...
                break;
            case operation::TYPE: as<type_info>(to) = type_id<Type>(); break;
            }
        }
        else
        {
            ...
            case operation::COPY:
                if constexpr(std::is_copy_constructible_v<Type>)
                {
                    as<basic_any>(to).instance =
                            new Type{*static_cast<const Type*>(from.instance)};
                    as<basic_any>(to).vtable = from.vtable;
                }
                break;
            case operation::MOVE:
                as<basic_any>(to).instance =
                        std::exchange(as<basic_any>(&from).instance, nullptr);
                as<basic_any>(to).vtable = from.vtable;
                break;
            case operation::DTOR:
...
public:
    ...
    basic_any() ENTT_NOEXCEPT : basic_any{std::in_place_type<void>} {}
    ...
    explicit basic_any(std::in_place_type_t<Type>, Args&&... args) :
        instance{}, vtable{&basic_vtable<Type>}
    {
    ...
    template<
            ...
            typename = std::enable_if_t<
                    !std::is_same_v<std::decay_t<Type>, basic_any>>>
     ...
     */
    basic_any(const basic_any& other) : basic_any{std::in_place_type<void>}
    {
    ...
    ~basic_any() { vtable(operation::DTOR, *this, nullptr); }
     ...
     */
    template<typename Type>
    std::enable_if_t<!std::is_same_v<std::decay_t<Type>, basic_any>, basic_any&> operator=(
            Type&& value)
    {
    ...
    bool operator==(const basic_any& other) const ENTT_NOEXCEPT
    {
        return type() == other.type()
               && (vtable(operation::COMP, *this, other.data())
                   == other.data());
    }
...
[[nodiscard]] inline bool operator!=(
        const basic_any<Len, Align>& lhs,
        const basic_any<Len, Align>& rhs) ENTT_NOEXCEPT
{
    return !(lhs == rhs);
}
...
{
    // forces const on non-reference types to make them work also with wrappers
...
{
    return (data->type() == type_id<Type>()
                    ? static_cast<const Type*>(data->data())
                    : nullptr);
}
...
Type* any_cast(basic_any<Len, Align>* data) ENTT_NOEXCEPT
{
    ...
    return (data->type() == type_id<Type>() ? static_cast<Type*>(
                    static_cast<constness_as_t<basic_any<Len, Align>, Type>*>(
                            data)
                            ->data())
                                            : nullptr);
}

...
[[nodiscard]] constexpr auto stripped_type_name() ENTT_NOEXCEPT
{
    ...
    auto value = pretty_function.substr(
            first,
            pretty_function.find_last_of(ENTT_PRETTY_FUNCTION_SUFFIX) - first);
    return value;
#else
...
[[nodiscard]] static constexpr id_type type_hash(int) ENTT_NOEXCEPT
{
    ...
    constexpr auto value =
            hashed_string::value(stripped.data(), stripped.size());
    return value;
}
...
class type_info final
{
    ...
    type_info(id_type seq_v, id_type hash_v, std::string_view name_v)
            ENTT_NOEXCEPT :
        seq_value{seq_v},
        hash_value{hash_v},
        name_value{name_v}
...
public:
    ...
    type_info() ENTT_NOEXCEPT : type_info({}, {}, {}) {}
     ...
     */
    [[nodiscard]] id_type seq() const ENTT_NOEXCEPT { return seq_value; }

    /**
     * @brief Type hash.
     * @return Type hash.
     */
    [[nodiscard]] id_type hash() const ENTT_NOEXCEPT { return hash_value; }
 ...
 */
[[nodiscard]] inline bool operator!=(const type_info& lhs, const type_info& rhs)
        ENTT_NOEXCEPT
{
    return !(lhs == rhs);
}

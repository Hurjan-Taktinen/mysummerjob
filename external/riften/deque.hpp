...
// This (standalone) file implements the deque described in the papers, "Correct
// and Efficient Work-Stealing for Weak Memory Models," and "Dynamic Circular
// Work-Stealing Deque". Both are avaliable in 'reference/'.
...
public:
    explicit RingBuff(std::int64_t cap) : _cap{cap}, _mask{cap - 1}
    {
        assert(cap && (!(cap & (cap - 1)))
               && "Capacity must be buf power of 2!");
    }
    ...
    {
        _buff[i & _mask].store(x, std::memory_order_relaxed);
    }

    // Relaxed load at modulo index
    T load(std::int64_t i) const noexcept
    {
        return _buff[i & _mask].load(std::memory_order_relaxed);
    }
...
private:
    ...
    std::unique_ptr<std::atomic<T>[]> _buff =
            std::make_unique<std::atomic<T>[]>(_cap);
#else
    std::unique_ptr<std::atomic<T>[]> _buff =
            std::make_unique_for_overwrite<std::atomic<T>[]>(_cap);
#endif
};
...
// Lock-free single-producer multiple-consumer deque. There are no constraints
// on the type `T` that can be stored. Only the deque owner can perform pop and
// push operations where the deque behaves like a stack. Others can (only) steal
// data from the deque, they see a FIFO queue. All threads must have finished
// using the deque before it is destructed.
//
// When `is_nothrow_move_constructible_v<T> == true`, Deque provides the strong
// exception garantee for all its methods.
template<typename T>
class Deque
{
public:
    // Constructs the deque with a given capacity the capacity of the deque
    // (must be power of 2)
    ...
    // Pops out an item from the deque. Only the owner thread can pop out an
    // item from the deque. The return can be a std::nullopt if this operation
    // failed (empty deque).
    std::optional<T> pop() noexcept(
            no_alloc || std::is_nothrow_move_constructible_v<T>);

    // Steals an item from the deque Any threads can try to steal an item from
    // the deque. The return can be a std::nullopt if this operation failed (not
    // necessary empty).
...
private:
    static constexpr bool no_alloc =
            std::is_trivially_destructible_v<T> && detail::lock_free_v<T>;
        ...
        no_alloc || std::is_nothrow_move_constructible_v<T>)
{
    ...
    if(t <= b)
    {
        ...
        // Can delay load until after aquiring slot as only this thread can
        // push()
        auto x = buf->load(b);
        ...
        no_alloc || std::is_nothrow_move_constructible_v<T>)
{
    ...
    if(t < b)
    {
        // Must load *before* aquiring the slot as slot may be overwritten
        // immidiatly after aquiring.

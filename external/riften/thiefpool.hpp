...
// Lightweight, fast, work-stealing thread-pool for C++20. Built on the
// lock-free concurrent `riften::Deque`. Upon destruction the threadpool blocks
// until all tasks have been completed and all threads have joined.
class Thiefpool
{
public:
            ...
            _threads.emplace_back([&, id = i](std::stop_token tok) {
                ...
                do
                {
                    ...
                    do
                    {
                        ...
                        std::size_t t =
                                spin++ < 100 || !_deques[id].tasks.empty()
                                        ? id
    ...
    // Enqueue callable `f` into the threadpool. Like `std::async`/`std::thread`
    // a copy of `args...` is made, use `std::ref` if you really want a
    // reference. Returns a `std::future<...>` which does not block upon
    // destruction.
    template<typename... Args, typename F>
    ...
    {
        //
        auto task = detail::NullaryOneShot(
                detail::bind(std::forward<F>(f), std::forward<Args>(args)...));
    ...
    // Enqueue callable `f` into the threadpool. Like `std::async`/`std::thread`
    // a copy of `args...` is made, use `std::ref` if you really want a
    // reference. This version does *not* return a handle to the called function
